...
const DFY_DIRTY_PRODUCT_MONTHS_KEY = "dfy-product-ledger-dirty-months-v1";
...
const DFY_PRODUCT_PENDING_GRACE_MS = 3000;
const DFY_SUPABASE_PAGE_SIZE = 1000;
const DFY_SUPABASE_WRITE_CHUNK_SIZE = 500;
...
const DFY_PAGE_ORDER = ["summary", "products", "inventory", "inventoryOverview", "cashflow"];
const DFY_CASHFLOW_CATEGORY_ORDER = ["labor", "taxAccounting", "insurance", "rent", "purchaseCost", "supplies", "meal", "shipping", "transport", "ads", "sample", "other", "card", "tax", "taxReserve"];
const DFY_SETTLEMENT_PENDING_CATEGORIES = ["무신사", "무신사 글로벌", "에이블리", "크림", "자사몰", "오프라인"];
...
const DFY_PAGE_META = {
  ...
  products: {
    chip: "02 PAGE",
    name: "상품별 일일 매출 현황",
    description: "카테고리별 상품 판매건수를 일자와 사이즈 기준으로 입력합니다. 입력값은 로컬에 유지되며, 저장 버튼으로 공유 저장됩니다."
  },
  inventory: {
    chip: "03 PAGE",
    name: "입출고 내역",
  ...
  cashflow: {
    chip: "05 PAGE",
    name: "현금흐름표",
    description: "현금 유입과 유출을 정리할 수 있는 신규 페이지입니다. 구조화는 이후 단계에서 순차적으로 확장할 수 있도록 기본 틀만 먼저 개설했습니다."
  }
};

const DFY_CASHFLOW_CATEGORIES = {
  labor: { label: "인건비" },
  taxAccounting: { label: "세무비용" },
  insurance: { label: "보험비" },
  rent: { label: "월세" },
  purchaseCost: { label: "매입원가", subcategories: ["원단", "봉제", "나염", "부자재", "모자", "소품"] },
  supplies: { label: "비품" },
  meal: { label: "식비" },
  shipping: { label: "택배비" },
  transport: { label: "교통비" },
  ...
  sample: { label: "샘플비" },
  other: { label: "기타비용" },
  card: { label: "카드값", subcategories: ["식비", "비품", "교통비", "샘플비", "접대비", "핸드폰비", "대식카드", "종호카드", "대중교통비", "광고비"] },
  tax: { label: "세금" },
  taxReserve: { label: "세금예비비" }
};
...
const productAddItemButton = document.getElementById("productAddItemButton");
const productRemoveItemButton = document.getElementById("productRemoveItemButton");
...
const inventoryAddSeasonButton = document.getElementById("inventoryAddSeasonButton");
const inventoryRemoveSeasonButton = document.getElementById("inventoryRemoveSeasonButton");
const inventoryAddItemButton = document.getElementById("inventoryAddItemButton");
const inventoryRemoveItemButton = document.getElementById("inventoryRemoveItemButton");
...
const cashflowBusinessBalanceValue = document.getElementById("cashflowBusinessBalanceValue");
const cashflowBusinessBalanceFootValue = document.getElementById("cashflowBusinessBalanceFootValue");
const cashflowNetBalanceValue = document.getElementById("cashflowNetBalanceValue");
const cashflowBusinessAccountInput = document.getElementById("cashflowBusinessAccountInput");
const cashflowSettlementPendingInput = document.getElementById("cashflowSettlementPendingInput");
const cashflowOtherIncomeInput = document.getElementById("cashflowOtherIncomeInput");
const cashflowNextMonthTotalValue = document.getElementById("cashflowNextMonthTotalValue");
const cashflowPayablesAddButton = document.getElementById("cashflowPayablesAddButton");
const cashflowSettlementConfirmedAddButton = document.getElementById("cashflowSettlementConfirmedAddButton");
const cashflowPayablesBody = document.getElementById("cashflowPayablesBody");
const cashflowSettlementConfirmedBody = document.getElementById("cashflowSettlementConfirmedBody");
const cashflowSettlementCategoryList = document.getElementById("cashflowSettlementCategoryList");
const cashflowCategoryButtons = document.getElementById("cashflowCategoryButtons");
const cashflowExpenseTitle = document.getElementById("cashflowExpenseTitle");
const cashflowExpenseCaption = document.getElementById("cashflowExpenseCaption");
const cashflowSubcategoryChips = document.getElementById("cashflowSubcategoryChips");
const cashflowAddRowButton = document.getElementById("cashflowAddRowButton");
const cashflowRecordsHead = document.getElementById("cashflowRecordsHead");
const cashflowRecordsBody = document.getElementById("cashflowRecordsBody");
...
let currentProductCategory = normalizeCategoryKey(state.ui?.currentProductCategory, state.customCatalog);
let currentInventoryCategory = normalizeInventoryCategoryKey(state.ui?.currentInventoryCategory);
let currentInventorySeason = normalizeInventorySeasonKey(currentInventoryCategory, state.ui?.currentInventorySeason, state.customCatalog);
...
let currentInventoryDate = todayKey;
let currentCashflowCategory = normalizeCashflowCategoryKey(state.ui?.currentCashflowCategory);
...
let suppressProductBlurCommit = false;
let skipInventoryBlurCommit = false;
let isInventoryEditModalOpen = false;
let productRealtimeChannel = null;
let isProductSaveInFlight = false;
let dirtyProductMonths = new Set();
let isProductSelectionDragging = false;
let productSelectionAnchor = null;
let productSelectionMoved = false;
let productSelectionRange = null;
let productUndoStack = [];
let selectedProductItemKey = "";
let selectedInventoryItemId = "";
let selectedInventoryOverviewItemId = "";
let selectedInventoryOverviewSeasonKey = "";
let activeCatalogDrag = null;
...
function buildProductRows(categories = getProductCategories()) {
  return Object.entries(categories).flatMap(([categoryKey, category]) => {
    return category.items.flatMap((item, itemIndex) => {
      return item.sizes.map((size, sizeIndex) => ({
        ...
        itemKey: `${categoryKey}:${slugify(item.name)}`,
        categoryKey,
...
function loadState() {
  ...
  if (!stored) {
      return {
        ...
        meta: {},
        productLedger: {},
        customCatalog: createEmptyCustomCatalog(),
        cashflow: {},
        ui: {
          ...
          currentCashflowCategory: getDefaultCashflowCategoryKey()
        }
      };
  }
  ...
  try {
    ...
    const parsedCustomCatalog = normalizeCustomCatalog(parsed.customCatalog);
      ...
      const parsedMeta = Object.fromEntries(
        Object.entries(parsed.meta || {}).map(([monthKey, meta]) => [
          monthKey,
          {
            ...
            productAverageDivisor: Math.max(0, Number(meta?.productAverageDivisor) || 0),
            cashflowBusinessAccount: Math.max(0, Number(meta?.cashflowBusinessAccount) || 0),
            cashflowSettlementPending: Math.max(0, Number(meta?.cashflowSettlementPending) || 0),
            cashflowOtherIncome: Math.max(0, Number(meta?.cashflowOtherIncome) || 0),
            cashflowPayables: Math.max(0, Number(meta?.cashflowPayables) || 0),
            cashflowSettlementConfirmed: Math.max(0, Number(meta?.cashflowSettlementConfirmed) || 0),
            cashflowSettlementPendingBreakdown: normalizeSettlementPendingBreakdown(meta?.cashflowSettlementPendingBreakdown),
            cashflowNextMonthPayablesEntries: normalizeNextMonthEntries(meta?.cashflowNextMonthPayablesEntries, monthKey, "payables", meta?.cashflowPayables),
            cashflowNextMonthSettlementConfirmedEntries: normalizeNextMonthEntries(meta?.cashflowNextMonthSettlementConfirmedEntries, monthKey, "settlement-confirmed", meta?.cashflowSettlementConfirmed)
          }
        ])
      );
    ...
    return {
      ...
      customCatalog: parsedCustomCatalog,
      cashflow: normalizeCashflowState(parsed.cashflow),
      ui: {
        currentMonth: normalizeMonthKey(parsed.ui?.currentMonth) || todayMonthKey,
        currentPage: normalizePageKey(parsed.ui?.currentPage),
        currentProductCategory: normalizeCategoryKey(parsed.ui?.currentProductCategory, parsedCustomCatalog),
        ...
        currentCashflowCategory: normalizeCashflowCategoryKey(parsed.ui?.currentCashflowCategory)
      }
    };
  } catch (error) {
    return {
...
function createEmptyCustomCatalog() {
  return { products: {}, inventory: {}, seasons: {}, removedSeasons: {}, removedProducts: {}, removedInventory: {}, productOrder: {}, inventoryOrder: {} };
}

function hasCustomCatalogData(customCatalog) {
  const normalized = normalizeCustomCatalog(customCatalog);
  return Object.values(normalized.products).some((items) => items.length)
    || Object.values(normalized.inventory).some((seasonMap) => Object.values(seasonMap).some((items) => items.length))
    || Object.values(normalized.seasons).some((seasonMap) => Object.keys(seasonMap).length)
    || Object.values(normalized.removedSeasons).some((seasonKeys) => seasonKeys.length)
    || Object.values(normalized.removedProducts).some((items) => items.length)
    || Object.values(normalized.removedInventory).some((seasonMap) => Object.values(seasonMap).some((items) => items.length))
    || Object.values(normalized.productOrder).some((items) => items.length)
    || Object.values(normalized.inventoryOrder).some((seasonMap) => Object.values(seasonMap).some((items) => items.length));
}

function normalizeCustomCatalog(customCatalog) {
  const nextCatalog = createEmptyCustomCatalog();
  const source = customCatalog && typeof customCatalog === "object" ? customCatalog : {};

  const productCategoryKeys = Array.from(new Set([
    ...Object.keys(DFY_PRODUCT_CATEGORIES),
    ...Object.keys(DFY_INVENTORY_CATEGORIES),
    ...Object.keys(source.products || {})
  ]));
  productCategoryKeys.forEach((categoryKey) => {
    const items = Array.isArray(source.products?.[categoryKey]) ? source.products[categoryKey] : [];
    nextCatalog.products[categoryKey] = dedupeProducts(
      items
        .map((item) => normalizeCustomProductItem(item))
        .filter(Boolean)
    );
    nextCatalog.productOrder[categoryKey] = Array.isArray(source.productOrder?.[categoryKey])
      ? Array.from(new Set(source.productOrder[categoryKey].map((value) => String(value || "").trim()).filter(Boolean)))
      : [];
    nextCatalog.removedProducts[categoryKey] = Array.isArray(source.removedProducts?.[categoryKey])
      ? Array.from(new Set(source.removedProducts[categoryKey].map((value) => String(value || "").trim()).filter(Boolean)))
      : [];
  });

  Object.keys(DFY_INVENTORY_CATEGORIES).forEach((categoryKey) => {
    nextCatalog.inventory[categoryKey] = {};
    nextCatalog.seasons[categoryKey] = {};
    nextCatalog.removedSeasons[categoryKey] = Array.isArray(source.removedSeasons?.[categoryKey])
      ? Array.from(new Set(source.removedSeasons[categoryKey].map((value) => String(value || "").trim()).filter(Boolean)))
      : [];
    nextCatalog.removedInventory[categoryKey] = {};
    nextCatalog.inventoryOrder[categoryKey] = {};
    const baseSeasons = DFY_INVENTORY_CATEGORIES[categoryKey]?.seasons || {};
    Object.entries(source.seasons?.[categoryKey] || {}).forEach(([seasonKey, season]) => {
      if (seasonKey in baseSeasons) {
        return;
      }
      const normalizedSeason = normalizeCustomSeasonDefinition(season, DFY_INVENTORY_CATEGORIES[categoryKey]?.label);
      if (normalizedSeason) {
        nextCatalog.seasons[categoryKey][seasonKey] = normalizedSeason;
      }
    });
    const seasonKeys = Array.from(new Set([
      ...Object.keys(baseSeasons),
      ...Object.keys(nextCatalog.seasons[categoryKey])
    ]));
    seasonKeys.forEach((seasonKey) => {
      const items = Array.isArray(source.inventory?.[categoryKey]?.[seasonKey])
        ? source.inventory[categoryKey][seasonKey]
        : [];
      nextCatalog.inventory[categoryKey][seasonKey] = dedupeInventoryItems(
        items
          .map((item) => normalizeCustomInventoryItem(item))
          .filter(Boolean)
      );
      nextCatalog.removedInventory[categoryKey][seasonKey] = Array.isArray(source.removedInventory?.[categoryKey]?.[seasonKey])
        ? Array.from(new Set(source.removedInventory[categoryKey][seasonKey].map((value) => String(value || "").trim()).filter(Boolean)))
        : [];
      nextCatalog.inventoryOrder[categoryKey][seasonKey] = Array.isArray(source.inventoryOrder?.[categoryKey]?.[seasonKey])
        ? Array.from(new Set(source.inventoryOrder[categoryKey][seasonKey].map((value) => String(value || "").trim()).filter(Boolean)))
        : [];
    });
  });
...
function mergeCustomCatalog(baseCatalog, incomingCatalog) {
  const base = normalizeCustomCatalog(baseCatalog);
  const incoming = normalizeCustomCatalog(incomingCatalog);
  const merged = createEmptyCustomCatalog();
  const productCategoryKeys = Array.from(new Set([
    ...Object.keys(base.products),
    ...Object.keys(incoming.products)
  ]));

  productCategoryKeys.forEach((categoryKey) => {
    merged.products[categoryKey] = dedupeProducts([
      ...(base.products[categoryKey] || []),
      ...(incoming.products[categoryKey] || [])
    ]);
    merged.productOrder[categoryKey] = mergeCatalogOrderLists(
      base.productOrder[categoryKey],
      incoming.productOrder[categoryKey]
    );
    merged.removedProducts[categoryKey] = Array.from(new Set([
      ...(base.removedProducts[categoryKey] || []),
      ...(incoming.removedProducts[categoryKey] || [])
    ]));
  });

  Object.keys(DFY_INVENTORY_CATEGORIES).forEach((categoryKey) => {
    merged.seasons[categoryKey] = {
      ...(base.seasons[categoryKey] || {}),
      ...(incoming.seasons[categoryKey] || {})
    };
    merged.removedSeasons[categoryKey] = Array.from(new Set([
      ...(base.removedSeasons[categoryKey] || []),
      ...(incoming.removedSeasons[categoryKey] || [])
    ]));
    merged.inventory[categoryKey] = {};
    merged.inventoryOrder[categoryKey] = {};
    merged.removedInventory[categoryKey] = {};
    const seasonKeys = Array.from(new Set([
      ...Object.keys(base.inventory[categoryKey] || {}),
      ...Object.keys(incoming.inventory[categoryKey] || {}),
      ...Object.keys(merged.seasons[categoryKey])
    ]));
    seasonKeys.forEach((seasonKey) => {
      merged.inventory[categoryKey][seasonKey] = dedupeInventoryItems([
        ...(base.inventory[categoryKey]?.[seasonKey] || []),
        ...(incoming.inventory[categoryKey]?.[seasonKey] || [])
      ]);
      merged.inventoryOrder[categoryKey][seasonKey] = mergeCatalogOrderLists(
        base.inventoryOrder[categoryKey]?.[seasonKey],
        incoming.inventoryOrder[categoryKey]?.[seasonKey]
      );
      merged.removedInventory[categoryKey][seasonKey] = Array.from(new Set([
        ...(base.removedInventory[categoryKey]?.[seasonKey] || []),
        ...(incoming.removedInventory[categoryKey]?.[seasonKey] || [])
      ]));
    });
  });

  return normalizeCustomCatalog(merged);
}

function mergeCatalogOrderLists(baseOrder = [], incomingOrder = []) {
  return Array.from(new Set([
    ...(Array.isArray(incomingOrder) ? incomingOrder : []),
    ...(Array.isArray(baseOrder) ? baseOrder : [])
  ].map((value) => String(value || "").trim()).filter(Boolean)));
}

function normalizeCustomSeasonDefinition(season, categoryLabel = "") {
  const label = String(season?.label || season?.inventorySeason || "").trim();
  if (!label) {
    return null;
  }
  return {
    label,
    inventoryCategory: String(season?.inventoryCategory || categoryLabel).trim() || categoryLabel,
    inventorySeason: String(season?.inventorySeason || label).trim() || label
  };
}

function normalizeCustomProductItem(item) {
  const name = String(item?.name || "").trim();
  if (!name) {
    return null;
  }
  const sizes = normalizeSizeList(item?.sizes, ["M", "L"]);
  return createProductDefinition(name, sizes, {
    inventoryName: String(item?.inventoryName || name).trim() || name,
    syncSizeMap: item?.syncSizeMap && typeof item.syncSizeMap === "object" ? { ...item.syncSizeMap } : {}
  });
}
...
function normalizeCustomInventoryItem(item) {
  ...
  const sizes = normalizeSizeList(item?.sizes, ["M", "L"]);
  const defaultStock = typeof item?.defaultStock === "object" && item.defaultStock !== null
    ? Object.fromEntries(sizes.map((size) => [size, Math.max(0, Number(item.defaultStock[size] ?? item.defaultStock.OS ?? 0) || 0)]))
    : Object.fromEntries(sizes.map((size) => [size, 0]));
  return createInventoryDefinition(name, sizes, {
    inventoryName: String(item?.inventoryName || name).trim() || name,
    defaultStock
  });
}

function normalizeSizeList(sizes, fallbackSizes = ["M", "L"]) {
  const values = Array.isArray(sizes) ? sizes : [];
  const nextSizes = values
    .map((size) => String(size || "").trim().toUpperCase())
    .filter(Boolean);
  return nextSizes.length ? Array.from(new Set(nextSizes)) : [...fallbackSizes];
}
...
function getProductCategories() {
  const customCatalog = normalizeCustomCatalog(state.customCatalog);
  const categoryKeys = Array.from(new Set([
    ...Object.keys(DFY_PRODUCT_CATEGORIES),
    ...Object.keys(customCatalog.products).filter((categoryKey) => customCatalog.products[categoryKey]?.length)
  ]));
  return Object.fromEntries(
    categoryKeys.map((categoryKey) => {
      const category = DFY_PRODUCT_CATEGORIES[categoryKey] || {
        label: DFY_INVENTORY_CATEGORIES[categoryKey]?.label || categoryKey,
        inventoryCategory: DFY_INVENTORY_CATEGORIES[categoryKey]?.label || categoryKey,
        items: []
      };
      return [categoryKey, {
        ...category,
        items: sortCatalogItemsByOrder(dedupeProducts([
          ...category.items,
          ...(customCatalog.products[categoryKey] || [])
        ]).filter((item) => !customCatalog.removedProducts[categoryKey]?.includes(createCatalogIdentity(item.name, item.sizes))), customCatalog.productOrder[categoryKey])
      }];
    })
  );
}
...
function getInventoryCategories() {
  const customCatalog = normalizeCustomCatalog(state.customCatalog);
  return Object.fromEntries(
    Object.entries(DFY_INVENTORY_CATEGORIES).map(([categoryKey, category]) => {
      const mergedSeasons = {
        ...category.seasons,
        ...Object.fromEntries(Object.entries(customCatalog.seasons[categoryKey] || {}).map(([seasonKey, season]) => [
          seasonKey,
          { ...season, items: [] }
        ]))
      };
      const removedSeasonKeys = new Set(customCatalog.removedSeasons[categoryKey] || []);
      return [categoryKey, {
        ...category,
        seasons: Object.fromEntries(
          Object.entries(mergedSeasons)
            .filter(([seasonKey]) => !removedSeasonKeys.has(seasonKey))
            .map(([seasonKey, season]) => [
            seasonKey,
            {
              ...season,
              items: sortCatalogItemsByOrder(dedupeInventoryItems([
                ...season.items,
                ...(customCatalog.inventory[categoryKey]?.[seasonKey] || [])
              ]).filter((item) => !customCatalog.removedInventory[categoryKey]?.[seasonKey]?.includes(createCatalogIdentity(item.name, item.sizes))), customCatalog.inventoryOrder[categoryKey]?.[seasonKey])
            }
          ])
        )
      }];
...
function createCatalogIdentity(name, sizes = ["M", "L"]) {
  return `${slugify(name)}|${normalizeSizeList(sizes).join(",")}`;
}

function sortCatalogItemsByOrder(items, orderList = []) {
  if (!Array.isArray(orderList) || !orderList.length) {
    return [...items];
  }
  const orderMap = new Map(orderList.map((value, index) => [value, index]));
  return [...items].sort((left, right) => {
    const leftKey = createCatalogIdentity(left.name, left.sizes);
    const rightKey = createCatalogIdentity(right.name, right.sizes);
...
function loadDirtyProductMonths() {
  return Array.from(dirtyProductMonths);
}

function saveDirtyProductMonths(months) {
  dirtyProductMonths = new Set(months.map((value) => String(value)));
}

function markDirtyProductMonth(monthKey) {
  const nextMonths = loadDirtyProductMonths();
  if (!nextMonths.includes(monthKey)) {
    nextMonths.push(monthKey);
    saveDirtyProductMonths(nextMonths);
  }
}

function clearDirtyProductMonth(monthKey) {
  const nextMonths = loadDirtyProductMonths().filter((value) => value !== monthKey);
  saveDirtyProductMonths(nextMonths);
}

function isDirtyProductMonth(monthKey) {
  return loadDirtyProductMonths().includes(monthKey)
    || getPendingProductSyncEntries().some((entry) => entry.monthKey === monthKey);
}
...
function clearPendingProductSyncEntriesForMonth(monthKey) {
  const pendingMap = loadPendingProductSyncMap();
  const nextMap = Object.fromEntries(
    Object.entries(pendingMap).filter(([, entry]) => entry?.monthKey !== monthKey)
  );
  savePendingProductSyncMap(nextMap);
}
...
function getPendingProductSyncEntries() {
  return Object.values(loadPendingProductSyncMap())
    ...
    .filter((entry) => entry.quantity > 0 || entry.version >= 2);
}
...
function normalizeSharedInventoryState(inventoryState) {
  return {
    ...
    customCatalog: normalizeCustomCatalog(inventoryState?.customCatalog)
  };
}

function loadPendingInventorySyncEntry() {
  try {
    ...
    if (parsed && typeof parsed === "object" && "state" in parsed) {
      return {
        ...
        state: normalizeSharedInventoryState(parsed.state)
      };
    }
    return {
      changedAt: "",
      state: normalizeSharedInventoryState(parsed)
    };
  } catch (error) {
    return null;
  }
...
function savePendingInventorySyncState(inventoryState) {
  localStorage.setItem(
    ...
    JSON.stringify({
      changedAt: new Date().toISOString(),
      state: normalizeSharedInventoryState(inventoryState)
    })
  );
}
...
function bindEvents() {
  ...
  productAverageDivisor.addEventListener("focus", () => {
    productAverageDivisor.value = String(getMonthMeta(currentMonth).productAverageDivisor || getAverageDivisor(currentMonth));
  });
  productAverageDivisor.addEventListener("input", () => updateProductAverageDivisor(parseNumericValue(productAverageDivisor.value), false));
  productAverageDivisor.addEventListener("blur", () => updateProductAverageDivisor(parseNumericValue(productAverageDivisor.value), true));
  productAverageDivisor.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      productAverageDivisor.blur();
    }
  });

  cashflowBusinessAccountInput.addEventListener("focus", () => {
    cashflowBusinessAccountInput.value = String(getMonthMeta(currentMonth).cashflowBusinessAccount || "");
  });
  cashflowBusinessAccountInput.addEventListener("input", () => updateCashflowMeta("cashflowBusinessAccount", parseNumericValue(cashflowBusinessAccountInput.value), false));
  cashflowBusinessAccountInput.addEventListener("blur", () => updateCashflowMeta("cashflowBusinessAccount", parseNumericValue(cashflowBusinessAccountInput.value), true));
  cashflowBusinessAccountInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      cashflowBusinessAccountInput.blur();
    }
  });

  cashflowOtherIncomeInput.addEventListener("focus", () => {
    cashflowOtherIncomeInput.value = String(getMonthMeta(currentMonth).cashflowOtherIncome || "");
  });
  cashflowOtherIncomeInput.addEventListener("input", () => updateCashflowMeta("cashflowOtherIncome", parseNumericValue(cashflowOtherIncomeInput.value), false));
  cashflowOtherIncomeInput.addEventListener("blur", () => updateCashflowMeta("cashflowOtherIncome", parseNumericValue(cashflowOtherIncomeInput.value), true));
  cashflowOtherIncomeInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      cashflowOtherIncomeInput.blur();
    }
  });

  cashflowPayablesAddButton.addEventListener("click", () => {
    appendNextMonthEntry(currentMonth, "payables");
    saveState(`자동 저장됨 · ${formatTime(new Date())}`);
    renderCashflowPage();
  });

  cashflowSettlementConfirmedAddButton.addEventListener("click", () => {
    appendNextMonthEntry(currentMonth, "settlementConfirmed");
    saveState(`자동 저장됨 · ${formatTime(new Date())}`);
    renderCashflowPage();
  });

  cashflowCategoryButtons.addEventListener("click", (event) => {
    const button = event.target.closest("button[data-cashflow-category]");
    if (!button) {
      return;
    }
    currentCashflowCategory = normalizeCashflowCategoryKey(button.dataset.cashflowCategory);
    saveState(`자동 저장됨 · ${formatTime(new Date())}`);
    renderCashflowPage();
  });

  cashflowAddRowButton.addEventListener("click", () => {
    appendCashflowRecord(currentMonth, currentCashflowCategory);
    saveState(`자동 저장됨 · ${formatTime(new Date())}`);
    renderCashflowPage();
  });

  cashflowRecordsBody.addEventListener("input", handleCashflowRecordInput);
  cashflowRecordsBody.addEventListener("change", handleCashflowRecordInput);
  cashflowRecordsBody.addEventListener("blur", handleCashflowRecordBlur, true);
  cashflowRecordsBody.addEventListener("click", handleCashflowRecordDelete);
  cashflowPayablesBody.addEventListener("input", handleNextMonthEntryInput);
  cashflowPayablesBody.addEventListener("change", handleNextMonthEntryInput);
  cashflowPayablesBody.addEventListener("blur", handleNextMonthEntryBlur, true);
  cashflowPayablesBody.addEventListener("click", handleNextMonthEntryDelete);
  cashflowSettlementConfirmedBody.addEventListener("input", handleNextMonthEntryInput);
  cashflowSettlementConfirmedBody.addEventListener("change", handleNextMonthEntryInput);
  cashflowSettlementConfirmedBody.addEventListener("blur", handleNextMonthEntryBlur, true);
  cashflowSettlementConfirmedBody.addEventListener("click", handleNextMonthEntryDelete);

  if (cashflowSettlementCategoryList) {
    cashflowSettlementCategoryList.addEventListener("focusin", (event) => {
      const input = event.target;
      if (!(input instanceof HTMLInputElement) || !input.dataset.settlementCategory) {
        return;
      }
      const breakdown = normalizeSettlementPendingBreakdown(getMonthMeta(currentMonth).cashflowSettlementPendingBreakdown);
      input.value = String(breakdown[input.dataset.settlementCategory] || "");
    });
    cashflowSettlementCategoryList.addEventListener("input", (event) => {
      const input = event.target;
      if (!(input instanceof HTMLInputElement) || !input.dataset.settlementCategory) {
        return;
      }
      updateSettlementPendingBreakdown(input.dataset.settlementCategory, parseNumericValue(input.value), false);
    });
    cashflowSettlementCategoryList.addEventListener("blur", (event) => {
      const input = event.target;
      if (!(input instanceof HTMLInputElement) || !input.dataset.settlementCategory) {
        return;
      }
      updateSettlementPendingBreakdown(input.dataset.settlementCategory, parseNumericValue(input.value), true);
    }, true);
    cashflowSettlementCategoryList.addEventListener("keydown", (event) => {
      const input = event.target;
      if (!(input instanceof HTMLInputElement) || !input.dataset.settlementCategory) {
        return;
      }
  ...
  productSaveButton.addEventListener("click", () => {
    void handleProductSaveButtonClick();
  });
  productSaveButton.addEventListener("mousedown", (event) => {
    event.preventDefault();
  });
  productAddItemButton?.addEventListener("click", () => {
    void handleAddProductItem();
  });
  productRemoveItemButton?.addEventListener("click", () => {
    void handleRemoveProductItem();
  });

  productLedgerBody.addEventListener("focusin", handleProductInputFocus);
  productLedgerBody.addEventListener("click", handleProductRowSelectionClick);
  productLedgerBody.addEventListener("mousedown", handleProductSelectionMouseDown);
  productLedgerBody.addEventListener("mouseover", handleProductSelectionMouseOver);
  ...
  productLedgerBody.addEventListener("mousedown", handleProductDragMouseDown);
  document.addEventListener("mouseup", handleProductSelectionMouseUp);
  document.addEventListener("copy", handleProductSelectionCopy);
  document.addEventListener("keydown", handleProductSelectionDelete);
  document.addEventListener("keydown", handleProductUndoKeydown);
  ...
  inventorySeasonButtons.addEventListener("click", (event) => {
    ...
    currentInventorySeason = normalizeInventorySeasonKey(currentInventoryCategory, button.dataset.inventorySeason, state.customCatalog);
    saveState(`자동 저장됨 · ${formatTime(new Date())}`);
    renderInventorySeasonButtons();
    renderInventoryPage();
  });
  ...
  inventoryAddSeasonButton?.addEventListener("click", () => {
    void handleAddInventorySeason();
  });
  inventoryRemoveSeasonButton?.addEventListener("click", () => {
    void handleRemoveInventorySeason();
  });
  inventoryAddItemButton?.addEventListener("click", () => {
    void handleAddInventoryItem();
  });
  inventoryRemoveItemButton?.addEventListener("click", () => {
    void handleRemoveInventoryItem();
  });
  ...
  inventoryOverviewSections.addEventListener("click", handleInventoryOverviewSelectionClick);
  inventoryOverviewSections.addEventListener("mousedown", handleInventoryOverviewDragMouseDown);
  inventoryTableBody.addEventListener("focusin", handleInventoryInputFocus);
  inventoryTableBody.addEventListener("click", handleInventoryRowSelectionClick);
  ...
  inventoryTableBody.addEventListener("mousedown", handleInventoryDragMouseDown);
  document.addEventListener("dragstart", handleCatalogNativeDragStart);
  document.addEventListener("mousemove", handleCatalogDragMouseMove);
  document.addEventListener("mouseup", handleCatalogDragMouseUp);
...
function commitPendingInputsBeforeUnload() {
  ...
  if (activeElement.classList.contains("cashflow-record-input")) {
    if (activeElement instanceof HTMLSelectElement && activeElement.dataset.recordId) {
      handleCashflowRecordInput({ target: activeElement });
      return;
    }
    ...
    if (activeElement.dataset.nextMonthType && activeElement.dataset.recordId && activeElement.dataset.field) {
      handleNextMonthEntryInput({ target: activeElement });
      return;
    }

    if (activeElement.dataset.recordId && activeElement.dataset.field) {
      handleCashflowRecordInput({ target: activeElement });
    }
  }
}
...
function handleProductInputFocus(event) {
  ...
  selectedProductItemKey = getProductItemKeyFromRowId(input.dataset.rowId);
  input.value = String(getProductCellValue(currentMonth, input.dataset.rowId, input.dataset.date) || "");
  refreshProductSelectionStyles();
}

function handleProductRowSelectionClick(event) {
  const row = event.target instanceof Element ? event.target.closest("tr[data-item-key]") : null;
  if (!(row instanceof HTMLTableRowElement)) {
    return;
  }
  selectedProductItemKey = String(row.dataset.itemKey || "");
  refreshProductSelectionStyles();
}

function getProductSelectionCell(target) {
  const cell = target instanceof Element ? target.closest("td[data-grid-row][data-grid-col]") : null;
...
function clearProductRangeSelection() {
  productSelectionRange = null;
  productLedgerBody.querySelectorAll("td.is-range-selected").forEach((cell) => {
    cell.classList.remove("is-range-selected");
  });
}

function applyProductRangeSelection(startCell, endCell) {
  clearProductRangeSelection();
  ...
  productSelectionRange = { rowStart, rowEnd, colStart, colEnd };

  productLedgerBody.querySelectorAll("td[data-grid-row][data-grid-col]").forEach((cell) => {
    const row = Number(cell.dataset.gridRow);
    const col = Number(cell.dataset.gridCol);
    const isSelected = row >= rowStart && row <= rowEnd && col >= colStart && col <= colEnd;
    cell.classList.toggle("is-range-selected", isSelected);
  });
}

function applyProductRangeSelectionByBounds(range) {
  if (!range) {
    clearProductRangeSelection();
    return;
  }
  const startCell = productLedgerBody.querySelector(`td[data-grid-row="${range.rowStart}"][data-grid-col="${range.colStart}"]`);
  const endCell = productLedgerBody.querySelector(`td[data-grid-row="${range.rowEnd}"][data-grid-col="${range.colEnd}"]`);
  if (!(startCell instanceof HTMLTableCellElement) || !(endCell instanceof HTMLTableCellElement)) {
    clearProductRangeSelection();
    return;
  }
  applyProductRangeSelection(
    { cell: startCell, row: range.rowStart, col: range.colStart },
    { cell: endCell, row: range.rowEnd, col: range.colEnd }
  );
}

function pushProductUndoSnapshot(snapshot) {
  if (!snapshot?.changes?.length) {
    return;
  }
  productUndoStack.push(snapshot);
  if (productUndoStack.length > 30) {
    productUndoStack = productUndoStack.slice(-30);
  }
}

function handleProductSelectionMouseDown(event) {
  if (event.button !== 0) {
    return;
  }
  ...
  const nextAnchor = getProductSelectionCell(event.target);
  if (!nextAnchor) {
    return;
  }
  isProductSelectionDragging = true;
  productSelectionMoved = false;
  productSelectionAnchor = nextAnchor;
  applyProductRangeSelection(nextAnchor, nextAnchor);
}

function handleProductSelectionMouseOver(event) {
  if (!isProductSelectionDragging || !productSelectionAnchor) {
    return;
  }
  const hoveredCell = getProductSelectionCell(event.target);
  if (!hoveredCell) {
    return;
  }
  if (hoveredCell.row !== productSelectionAnchor.row || hoveredCell.col !== productSelectionAnchor.col) {
    productSelectionMoved = true;
  }
  applyProductRangeSelection(productSelectionAnchor, hoveredCell);
}

function handleProductSelectionMouseUp() {
  if (!isProductSelectionDragging) {
    return;
  }
  if (!productSelectionMoved && productSelectionAnchor?.cell) {
    const input = productSelectionAnchor.cell.querySelector("input.product-input");
    if (input instanceof HTMLInputElement) {
      input.focus();
      input.select();
    }
  }
  isProductSelectionDragging = false;
}

function handleProductSelectionCopy(event) {
  if (currentPage !== "products" || !productSelectionRange) {
    return;
  }

  const { rowStart, rowEnd, colStart, colEnd } = productSelectionRange;
  const rows = [];

  for (let row = rowStart; row <= rowEnd; row += 1) {
    const values = [];
    for (let col = colStart; col <= colEnd; col += 1) {
      const cell = productLedgerBody.querySelector(`td[data-grid-row="${row}"][data-grid-col="${col}"]`);
      const input = cell?.querySelector("input.product-input");
...
function handleProductSelectionDelete(event) {
  if (currentPage !== "products" || !productSelectionRange) {
    return;
  }
  ...
  for (let row = rowStart; row <= rowEnd; row += 1) {
    for (let col = colStart; col <= colEnd; col += 1) {
      ...
      if (getProductCellValue(currentMonth, rowId, dateKey) === 0) {
        continue;
      }
      ...
      changes.push({
        rowId,
        dateKey,
        previousValue: getProductCellValue(currentMonth, rowId, dateKey)
      });
      setProductCellValue(currentMonth, rowId, dateKey, 0);
      markPendingProductSyncEntry(currentMonth, rowId, dateKey);
      input.value = "";
      hasChanges = true;
    }
  }
  ...
  event.preventDefault();
  pushProductUndoSnapshot({
    monthKey: currentMonth,
    range: { rowStart, rowEnd, colStart, colEnd },
    changes
  });
  markDirtyProductMonth(currentMonth);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  renderProductPage();
  applyProductRangeSelectionByBounds({ rowStart, rowEnd, colStart, colEnd });
}

function handleProductUndoKeydown(event) {
  if (currentPage !== "products") {
    return;
  }

  const isUndo = (event.ctrlKey || event.metaKey) && !event.shiftKey && event.key.toLowerCase() === "z";
  if (!isUndo || event.altKey) {
    return;
  }

  const snapshot = productUndoStack.pop();
  if (!snapshot || snapshot.monthKey !== currentMonth) {
    if (snapshot) {
      productUndoStack.push(snapshot);
    }
    return;
  ...
  snapshot.changes.forEach((change) => {
    setProductCellValue(currentMonth, change.rowId, change.dateKey, change.previousValue);
    markPendingProductSyncEntry(currentMonth, change.rowId, change.dateKey);
  });
  markDirtyProductMonth(currentMonth);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  renderProductPage();
  applyProductRangeSelectionByBounds(snapshot.range);
}
...
function handleProductInputBlur(event) {
  const input = event.target;
  if (!(input instanceof HTMLInputElement) || !input.dataset.rowId || !input.dataset.date) return;
  if (suppressProductBlurCommit) return;
...
function handleInventoryInputFocus(event) {
  ...
  selectedInventoryItemId = String(input.dataset.itemId || "");
  ...
  refreshInventorySelectionStyles();
}

function handleInventoryRowSelectionClick(event) {
  const row = event.target instanceof Element ? event.target.closest("tr[data-item-id]") : null;
  if (!(row instanceof HTMLTableRowElement)) {
    return;
  }
  selectedInventoryItemId = String(row.dataset.itemId || "");
  refreshInventorySelectionStyles();
}

function handleInventoryOverviewSelectionClick(event) {
  const addButton = event.target instanceof Element ? event.target.closest("button[data-inventory-overview-add]") : null;
  if (addButton instanceof HTMLButtonElement) {
    void handleAddInventoryOverviewItem(addButton.dataset.inventoryOverviewAdd);
    return;
  }

  const removeButton = event.target instanceof Element ? event.target.closest("button[data-inventory-overview-remove]") : null;
  if (removeButton instanceof HTMLButtonElement) {
    void handleRemoveInventoryOverviewItem(removeButton.dataset.inventoryOverviewRemove);
    return;
  }

  const row = event.target instanceof Element ? event.target.closest("tr[data-item-id][data-season-key]") : null;
  if (!(row instanceof HTMLTableRowElement)) {
    return;
  }
  selectedInventoryOverviewItemId = String(row.dataset.itemId || "");
  selectedInventoryOverviewSeasonKey = String(row.dataset.seasonKey || "");
  refreshInventoryOverviewSelectionStyles();
}

function refreshProductSelectionStyles() {
  productLedgerBody.querySelectorAll("tr[data-item-key]").forEach((row) => {
    row.classList.toggle("is-item-selected", row.dataset.itemKey === selectedProductItemKey);
  });
}

function refreshInventorySelectionStyles() {
  inventoryTableBody.querySelectorAll("tr[data-item-id]").forEach((row) => {
    row.classList.toggle("is-row-selected", row.dataset.itemId === selectedInventoryItemId);
  });
}

function refreshInventoryOverviewSelectionStyles() {
  inventoryOverviewSections.querySelectorAll("tr[data-item-id][data-season-key]").forEach((row) => {
    const matches = row.dataset.itemId === selectedInventoryOverviewItemId
      && row.dataset.seasonKey === selectedInventoryOverviewSeasonKey;
    row.classList.toggle("is-row-selected", matches);
  });
}

function clearCatalogDragClasses() {
  document.body.classList.remove("is-catalog-dragging");
  document.querySelectorAll("tr.is-dragging, tr.is-drop-before, tr.is-drop-after").forEach((row) => {
    row.classList.remove("is-dragging", "is-drop-before", "is-drop-after");
  });
}

function handleCatalogNativeDragStart(event) {
  if (event.target instanceof Element && event.target.closest(".drag-handle")) {
    event.preventDefault();
  }
}

function handleProductDragMouseDown(event) {
  const handle = event.target instanceof Element ? event.target.closest(".drag-handle[data-item-key]") : null;
  if (!(handle instanceof HTMLElement) || event.button !== 0) {
    return;
  }
  const row = handle.closest("tr[data-item-key]");
  if (!(row instanceof HTMLTableRowElement)) {
    return;
  }
  const itemKey = String(handle.dataset.itemKey || row.dataset.itemKey || "");
  if (!itemKey) {
    return;
  }
  event.preventDefault();
  clearCatalogDragClasses();
  activeCatalogDrag = {
    type: "product",
    sourceKey: itemKey,
    sourceSeasonKey: "",
...
function handleInventoryDragMouseDown(event) {
  const handle = event.target instanceof Element ? event.target.closest(".drag-handle[data-item-id]") : null;
  if (!(handle instanceof HTMLElement) || event.button !== 0) {
    return;
  }
  ...
  const itemId = String(handle.dataset.itemId || row.dataset.itemId || "");
  if (!itemId) {
    return;
  }
  event.preventDefault();
...
function handleInventoryOverviewDragMouseDown(event) {
  const handle = event.target instanceof Element ? event.target.closest(".drag-handle[data-item-id][data-season-key]") : null;
  if (!(handle instanceof HTMLElement) || event.button !== 0) {
    return;
  }
  const row = handle.closest("tr[data-item-id][data-season-key]");
  if (!(row instanceof HTMLTableRowElement)) {
    return;
  }
  const itemId = String(handle.dataset.itemId || row.dataset.itemId || "");
  const seasonKey = String(handle.dataset.seasonKey || row.dataset.seasonKey || "");
  if (!itemId || !seasonKey) {
    return;
  }
  event.preventDefault();
...
function findCatalogDragTarget(type, pointTarget, sourceSeasonKey = "") {
  if (!(pointTarget instanceof Element)) {
    return null;
  }
  if (type === "product") {
    return pointTarget.closest("tr[data-item-key]");
  }
  if (type === "inventory") {
    return pointTarget.closest("tr[data-item-id]");
  }
  const row = pointTarget.closest("tr[data-item-id][data-season-key]");
  if (!(row instanceof HTMLTableRowElement)) {
    return null;
  }
  return String(row.dataset.seasonKey || "") === sourceSeasonKey ? row : null;
}

function applyCatalogDropIndicator(targetRow, dropAfter) {
  document.querySelectorAll("tr.is-drop-before, tr.is-drop-after").forEach((row) => {
    if (row !== targetRow) {
  ...
  targetRow.classList.toggle("is-drop-before", !dropAfter);
  targetRow.classList.toggle("is-drop-after", dropAfter);
}

function handleCatalogDragMouseMove(event) {
  if (!activeCatalogDrag) {
    return;
  }
  event.preventDefault();
  activeCatalogDrag.moved = true;
  const pointTarget = document.elementFromPoint(event.clientX, event.clientY);
  const targetRow = findCatalogDragTarget(activeCatalogDrag.type, pointTarget, activeCatalogDrag.sourceSeasonKey);
  if (!(targetRow instanceof HTMLTableRowElement) || targetRow === activeCatalogDrag.sourceRow) {
    activeCatalogDrag.targetRow = null;
    applyCatalogDropIndicator(null, false);
    return;
  }
  const rect = targetRow.getBoundingClientRect();
  const dropAfter = event.clientY > (rect.top + rect.height / 2);
  activeCatalogDrag.targetRow = targetRow;
  activeCatalogDrag.dropAfter = dropAfter;
  applyCatalogDropIndicator(targetRow, dropAfter);
}

function handleCatalogDragMouseUp(event) {
  if (!activeCatalogDrag) {
    return;
  }
  const dragState = activeCatalogDrag;
  const fallbackTarget = document.elementFromPoint(event.clientX, event.clientY);
  const targetRow = dragState.targetRow instanceof HTMLTableRowElement
    ? dragState.targetRow
    : findCatalogDragTarget(dragState.type, fallbackTarget, dragState.sourceSeasonKey);
  const dropAfter = targetRow instanceof HTMLTableRowElement
    ? dragState.dropAfter
    : false;
  ...
  if (!(targetRow instanceof HTMLTableRowElement) || targetRow === dragState.sourceRow || !dragState.moved) {
    return;
  }

  const rect = targetRow.getBoundingClientRect();
  const clientY = dropAfter ? rect.bottom : rect.top;
  if (dragState.type === "product") {
    reorderProductItemsByDrag(
      dragState.sourceKey,
      String(targetRow.dataset.itemKey || ""),
      clientY,
      rect
    );
    return;
  ...
  if (dragState.type === "inventory") {
    reorderInventoryItemsByDrag(
      currentInventoryCategory,
      currentInventorySeason,
      dragState.sourceKey,
      String(targetRow.dataset.itemId || ""),
      clientY,
      rect
    );
    return;
...
function renderProductCategoryButtons() {
  productCategoryButtons.innerHTML = Object.entries(getProductCategories())
...
function renderInventoryCategoryButtons() {
  ...
  inventoryCategoryButtons.innerHTML = DFY_INVENTORY_CATEGORY_ORDER
    .filter((categoryKey) => categoryKey in categories)
...
function renderInventorySeasonButtons() {
  const seasons = getInventoryCategories()[currentInventoryCategory]?.seasons || {};
...
function renderSummary() {
  ...
  const breakEvenAchievementRate = metrics.breakEvenSales > 0 ? (metrics.currentSales / metrics.breakEvenSales) : 0;
  ...
  breakEvenSalesValue.textContent = `${formatCurrency(metrics.breakEvenSales)} (${formatPercent(breakEvenAchievementRate)})`;
...
function renderProductPage() {
  ...
  if (document.activeElement !== productAverageDivisor) {
    productAverageDivisor.value = String(divisor);
  }
...
function updateProductSaveButton() {
  const isDirty = isDirtyProductMonth(currentMonth);
  productSaveButton.disabled = !isDirty || isProductSaveInFlight;
  if (isProductSaveInFlight) {
    productSaveButton.textContent = "저장 중...";
    return;
  }
  productSaveButton.textContent = "저장";
}
...
function renderCashflowPage() {
  ...
  const category = DFY_CASHFLOW_CATEGORIES[currentCashflowCategory];
  const records = getCashflowRecords(currentMonth, currentCashflowCategory);
  refreshCashflowBusinessBalanceSummary(meta);
  cashflowExpenseTitle.textContent = `${category.label} 기록`;
  cashflowExpenseCaption.textContent = category.subcategories?.length
    ? `하위 카테고리: ${category.subcategories.join(", ")}`
    : "선택한 메인 카테고리의 일자별 지출 기록을 관리합니다.";
  cashflowAddRowButton.textContent = `${category.label} 행 추가`;
  renderCashflowSettlementBreakdown();
  renderCashflowCategoryButtons();
  renderCashflowNextMonthSection();
  renderCashflowSubcategoryChips(category);
  renderCashflowRecordsHead(category);
  renderCashflowRecords(records);
}

function renderCashflowCategoryButtons() {
  const totalExpenses = DFY_CASHFLOW_CATEGORY_ORDER.reduce((sum, categoryKey) => sum + getCashflowCategoryTotal(currentMonth, categoryKey), 0);
  cashflowCategoryButtons.innerHTML = DFY_CASHFLOW_CATEGORY_ORDER
    .map((categoryKey) => [categoryKey, DFY_CASHFLOW_CATEGORIES[categoryKey]])
    .map(([categoryKey, category]) => `
      <button
        class="cashflow-category-row${categoryKey === currentCashflowCategory ? " is-active" : ""}"
        type="button"
        data-cashflow-category="${categoryKey}"
        role="tab"
        aria-selected="${categoryKey === currentCashflowCategory ? "true" : "false"}"
      >
          <span class="cashflow-category-row-label">
            <span>${escapeHtml(category.label)}</span>
            ${category.subcategories?.length ? `<span class="cashflow-category-row-meta">${category.subcategories.length}개 하위 항목</span>` : ""}
          </span>
          <strong class="cashflow-category-row-amount">
            <span>${formatCurrency(getCashflowCategoryTotal(currentMonth, categoryKey))}</span>
            <span class="cashflow-category-row-share">(${formatPercent(totalExpenses ? getCashflowCategoryTotal(currentMonth, categoryKey) / totalExpenses : 0)})</span>
          </strong>
        </button>
      `).join("");
}

function renderCashflowSettlementBreakdown() {
  if (!cashflowSettlementCategoryList) {
    return;
  }
  const meta = getMonthMeta(currentMonth);
  const breakdown = normalizeSettlementPendingBreakdown(meta.cashflowSettlementPendingBreakdown);
  cashflowSettlementCategoryList.innerHTML = DFY_SETTLEMENT_PENDING_CATEGORIES.map((label) => `
    <label class="cashflow-breakdown-row" for="cashflowSettlementCategory-${escapeAttribute(label)}">
      <span class="cashflow-breakdown-row-label">${escapeHtml(label)}</span>
      <input
        id="cashflowSettlementCategory-${escapeAttribute(label)}"
        class="summary-inline-input cashflow-input cashflow-breakdown-input"
        type="text"
        inputmode="numeric"
        data-settlement-category="${escapeAttribute(label)}"
        value="${breakdown[label] ? formatNumber(breakdown[label]) : ""}"
        placeholder="0"
      >
    </label>
  `).join("");
...
function renderCashflowNextMonthSection() {
  renderNextMonthEntries("payables", cashflowPayablesBody, "아직 입력된 미지급금 기록이 없습니다. + 버튼으로 추가할 수 있습니다.");
  renderNextMonthEntries("settlementConfirmed", cashflowSettlementConfirmedBody, "아직 입력된 정산확정금액 기록이 없습니다. + 버튼으로 추가할 수 있습니다.");
}

function renderNextMonthEntries(typeKey, targetBody, emptyMessage) {
  if (!targetBody) {
    return;
  }
  const records = getNextMonthEntries(currentMonth, typeKey);
  if (!records.length) {
    targetBody.innerHTML = `<tr class="cashflow-empty-row"><td colspan="3">${emptyMessage}</td></tr>`;
    return;
  }

  targetBody.innerHTML = records.map((record) => `
    <tr data-next-month-type="${typeKey}" data-record-id="${record.id}">
      <td>
        <input
          class="cashflow-record-input amount"
          type="text"
          inputmode="numeric"
          data-next-month-type="${typeKey}"
          data-record-id="${record.id}"
          data-field="amount"
          value="${record.amount ? formatNumber(record.amount) : ""}"
          placeholder="0"
        >
      </td>
      <td class="cashflow-note-cell">
        <input
          class="cashflow-record-input"
          type="text"
          data-next-month-type="${typeKey}"
          data-record-id="${record.id}"
          data-field="note"
          value="${escapeAttribute(record.note)}"
          placeholder="비고를 입력하세요"
        >
      </td>
      <td>
        <button
          class="cashflow-delete-button"
          type="button"
          data-next-month-type="${typeKey}"
          data-record-id="${record.id}"
          aria-label="기록 삭제"
...
function renderCashflowSubcategoryChips(category) {
  cashflowSubcategoryChips.innerHTML = (category.subcategories || [])
    .map((label) => `<span class="cashflow-subcategory-chip">${escapeHtml(label)}</span>`)
    .join("");
}

function renderCashflowRecordsHead(category) {
  const hasSubcategories = Array.isArray(category.subcategories) && category.subcategories.length > 0;
  cashflowRecordsHead.innerHTML = `
    <tr>
      <th>일자</th>
      ${hasSubcategories ? "<th>하위 카테고리</th>" : ""}
      <th>금액</th>
      <th>비고</th>
      <th>삭제</th>
    </tr>
...
function renderCashflowRecords(records) {
  const category = DFY_CASHFLOW_CATEGORIES[currentCashflowCategory];
  const hasSubcategories = Array.isArray(category.subcategories) && category.subcategories.length > 0;
  if (!records.length) {
    cashflowRecordsBody.innerHTML = `<tr class="cashflow-empty-row"><td colspan="${hasSubcategories ? 5 : 4}">아직 입력된 지출 기록이 없습니다. 행 추가 버튼으로 시작할 수 있습니다.</td></tr>`;
    return;
  }

  cashflowRecordsBody.innerHTML = records.map((record) => `
    <tr data-record-id="${record.id}">
      <td>
        <input class="cashflow-record-input" type="date" data-record-id="${record.id}" data-field="date" value="${record.date}">
      </td>
      ${hasSubcategories ? `
      <td>
        <select class="cashflow-record-input" data-record-id="${record.id}" data-field="subcategory">
          ${category.subcategories.map((label) => `<option value="${escapeAttribute(label)}"${record.subcategory === label ? " selected" : ""}>${escapeHtml(label)}</option>`).join("")}
        </select>
      </td>
      ` : ""}
      <td>
        <input class="cashflow-record-input amount" type="text" inputmode="numeric" data-record-id="${record.id}" data-field="amount" value="${record.amount ? formatNumber(record.amount) : ""}" placeholder="0">
      </td>
      <td class="cashflow-note-cell">
        <input class="cashflow-record-input" type="text" data-record-id="${record.id}" data-field="note" value="${escapeAttribute(record.note)}" placeholder="비고를 입력하세요">
      </td>
      <td>
        <button class="cashflow-delete-button" type="button" data-record-id="${record.id}" aria-label="기록 삭제">삭제</button>
      </td>
    </tr>
  `).join("");
}
...
function renderInventoryOverviewSections() {
  ...
  const category = getInventoryCategories()[currentInventoryOverviewCategory];
  if (!category) {
    inventoryOverviewSections.innerHTML = "";
    return;
  }

  inventoryOverviewSections.innerHTML = Object.entries(category.seasons).map(([seasonKey, season]) => {
    const rows = season.items.map((item, itemIndex) => {
      ...
      const rowClass = record.id === selectedInventoryOverviewItemId ? " is-row-selected" : "";
      return `
        <tr class="${rowClass}" data-item-id="${record.id}" data-season-key="${seasonKey}">
          <th class="inventory-item-name"><span class="drag-handle" data-item-id="${record.id}" data-season-key="${seasonKey}" title="드래그로 순서 변경" aria-hidden="true">⋮⋮</span><span>${escapeHtml(record.name)}</span></th>
    ...
    return `
      <section class="inventory-season-card">
        <div class="inventory-season-card-top">
          ...
          <div class="inventory-season-card-side">
            ...
            <div class="catalog-action-row inventory-overview-actions" aria-label="${escapeHtml(season.label)} 상품 관리">
              <button class="catalog-action-button" type="button" data-inventory-overview-add="${seasonKey}">+ 상품추가</button>
              <button class="catalog-action-button is-danger" type="button" data-inventory-overview-remove="${seasonKey}">- 상품삭제</button>
            </div>
          </div>
        </div>
...
function calculateInventoryAverageDailySales(monthKey, item, size) {
  return getProductRows().reduce((sum, row) => {
    if (
...
function renderInventoryTable() {
  ...
  inventoryTableBody.innerHTML = season.items.map((item, itemIndex) => {
    ...
    const sizes = Array.isArray(record.sizes) && record.sizes.length ? record.sizes : [...item.sizes];
    const isOsOnly = sizes.length === 1 && sizes[0] === "OS";
    const rowClass = record.id === selectedInventoryItemId ? " is-row-selected" : "";

    return `
      <tr class="${rowClass}" data-item-id="${record.id}">
        <th class="inventory-item-name"><span class="drag-handle" data-item-id="${record.id}" title="드래그로 순서 변경" aria-hidden="true">⋮⋮</span><span>${escapeHtml(record.name)}</span></th>
        ${renderInventoryStockGroup(inventoryState, record, sizes, isOsOnly)}
...
function renderInventoryHistoryTable() {
  ...
  const rows = getInventoryTransactions(inventoryState)
    ...
    .sort((left, right) => {
      if ((left.category || "") === (right.category || "") && (left.season || "") === (right.season || "") && left.name === right.name) {
      ...
      if ((left.category || "") === (right.category || "")) {
        ...
        return (left.season || "").localeCompare(right.season || "", "ko");
      }
  ...
  inventoryHistoryBody.innerHTML = rows.length
    ...
    `).join("")
    : `<tr><td colspan="7">입출고 내역이 없습니다.</td></tr>`;
}
...
function renderProductLedgerBody() {
  ...
  const dateKeys = getDateKeysForMonth(currentMonth);
  productLedgerBody.innerHTML = rows.map((row, rowIndex) => {
    ...
    const rowClass = [row.itemIndex % 2 === 1 ? "group-alt" : "", row.itemKey === selectedProductItemKey ? "is-item-selected" : ""].filter(Boolean).join(" ");
    return `<tr class="${rowClass}" data-row-id="${row.rowId}" data-item-key="${row.itemKey}">${row.isFirstSize ? `<th class="product-sticky-col product-name-cell" rowspan="${row.rowSpan}"><span class="drag-handle" data-item-key="${row.itemKey}" title="드래그로 순서 변경" aria-hidden="true">⋮⋮</span><span>${escapeHtml(row.name)}</span></th>` : ""}<th class="product-sticky-col product-size-cell">${escapeHtml(row.size)}</th><td class="product-sticky-col product-total-cell" data-role="row-total">${formatCellValue(rowTotal)}</td><td class="product-sticky-col product-average-cell" data-role="row-average">${formatAverageCell(rowAverage, rowTotal)}</td>${dateKeys.map((dateKey, colIndex) => renderProductInputCell(row.rowId, row.name, row.size, dateKey, rowIndex, colIndex)).join("")}</tr>`;
  }).join("");
}

function renderProductInputCell(rowId, productName, size, dateKey, rowIndex, colIndex) {
  const value = getProductCellValue(currentMonth, rowId, dateKey);
  return `<td class="product-grid-cell" data-grid-row="${rowIndex}" data-grid-col="${colIndex}"><input class="product-input" type="text" inputmode="numeric" data-row-id="${rowId}" data-date="${dateKey}" placeholder="-" value="${value ? formatNumber(value) : ""}" aria-label="${formatDisplayDate(dateKey)} ${escapeHtml(productName)} ${size} 판매건수"></td>`;
}
...
function renderProductCategoryChart() {
  const categoryTotals = Object.entries(getProductCategories()).map(([categoryKey, category]) => ({
    categoryKey,
...
function ensureInventoryItemsExist() {
  ...
  if (hasChanges) {
    saveSharedInventoryState(inventoryState, { silentRemote: true, trackPending: false });
  }
...
function saveSharedInventoryState(inventoryState, options = {}) {
  const normalizedState = normalizeSharedInventoryState({
    ...inventoryState,
...
function getCurrentInventorySeasonConfig() {
  return getInventoryCategories()[currentInventoryCategory].seasons[currentInventorySeason];
}
...
function getInventoryDailySummary(dateKey) {
  ...
  const rows = getInventoryTransactions(inventoryState)
    ...
    .reduce((map, transaction) => {
      const key = `${transaction.category || ""}|${transaction.season || ""}|${transaction.name}|${transaction.size}`;
      if (!map.has(key)) {
        map.set(key, {
  ...
  const summaryRows = Array.from(rows.values()).sort((left, right) => {
    if ((left.category || "") === (right.category || "")) {
      if ((left.season || "") === (right.season || "")) {
...
function updateProductCellFromInput(input, shouldFormat) {
  ...
  input.dataset.productEdited = "true";
...
function commitVisibleProductInputsForMonth(monthKey) {
  if (!productLedgerBody) {
    return;
  }
  ...
  productLedgerBody.querySelectorAll("input.product-input[data-row-id][data-date]").forEach((input) => {
    if (!(input instanceof HTMLInputElement) || !input.dataset.date?.startsWith(monthKey)) {
      return;
    }
    const entryKey = createRemoteProductEntryKey(input.dataset.date, input.dataset.rowId);
    const pendingEntry = pendingMap[entryKey];
    const hasValidPendingEntry = Boolean(
      pendingEntry
      && (Math.max(0, Number(pendingEntry.quantity) || 0) > 0 || Math.max(1, Number(pendingEntry.version) || 1) >= 2)
    );
    if (input.dataset.productEdited !== "true" && !hasValidPendingEntry) {
      return;
    }
    const nextValue = parseNumericValue(input.value);
    setProductCellValue(monthKey, input.dataset.rowId, input.dataset.date, nextValue);
    markPendingProductSyncEntry(monthKey, input.dataset.rowId, input.dataset.date);
    delete input.dataset.productEdited;
    hasCommitted = true;
  });

  if (hasCommitted) {
    markDirtyProductMonth(monthKey);
    saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  }
}

function collectProductEntriesForSave(monthKey) {
  const pendingMap = loadPendingProductSyncMap();
  const entryMap = new Map();

  Object.entries(state.productLedger?.[monthKey] || {}).forEach(([rowId, dateMap]) => {
    Object.entries(dateMap || {}).forEach(([dateKey, quantity]) => {
      const normalizedQuantity = Math.max(0, Number(quantity) || 0);
      if (normalizedQuantity <= 0 || !dateKey.startsWith(monthKey)) {
        return;
      }
      entryMap.set(createRemoteProductEntryKey(dateKey, rowId), {
        monthKey,
        rowId,
        dateKey,
  ...
  getPendingProductSyncEntries()
    .filter((entry) => entry.monthKey === monthKey)
    .forEach((entry) => {
      entryMap.set(createRemoteProductEntryKey(entry.dateKey, entry.rowId), entry);
    });
  ...
  productLedgerBody.querySelectorAll("input.product-input[data-row-id][data-date]").forEach((input) => {
    if (!(input instanceof HTMLInputElement) || !input.dataset.rowId || !input.dataset.date?.startsWith(monthKey)) {
      return;
    }
    ...
    const shouldDelete = Boolean(
      pendingEntry
      && Math.max(1, Number(pendingEntry.version) || 1) >= 2
      && quantity <= 0
    );

    setProductCellValue(monthKey, rowId, dateKey, quantity);

    if (quantity > 0 || shouldDelete) {
...
function updateProductAverageDivisor(value, shouldFormat) {
  const meta = getMonthMeta(currentMonth);
  meta.productAverageDivisor = Math.max(0, Number(value) || 0);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);

  if (shouldFormat) {
    productAverageDivisor.value = String(getAverageDivisor(currentMonth));
    renderProductPage();
  }
}

function updateCashflowMeta(field, value, shouldFormat) {
  const meta = getMonthMeta(currentMonth);
  meta[field] = Math.max(0, Number(value) || 0);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  refreshCashflowBusinessBalanceSummary(meta);
  ...
  if (field === "cashflowBusinessAccount") {
    cashflowBusinessAccountInput.value = meta[field] ? formatNumber(meta[field]) : "";
    return;
  }
  cashflowOtherIncomeInput.value = meta[field] ? formatNumber(meta[field]) : "";
}

function updateSettlementPendingBreakdown(categoryLabel, value, shouldFormat) {
  const meta = getMonthMeta(currentMonth);
  const breakdown = normalizeSettlementPendingBreakdown(meta.cashflowSettlementPendingBreakdown);
  breakdown[categoryLabel] = Math.max(0, Number(value) || 0);
  meta.cashflowSettlementPendingBreakdown = breakdown;
  meta.cashflowSettlementPending = Object.values(breakdown).reduce((sum, amount) => sum + Math.max(0, Number(amount) || 0), 0);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  refreshCashflowBusinessBalanceSummary(meta);
  if (!shouldFormat || !cashflowSettlementCategoryList) {
    return;
  }
  const selector = `[data-settlement-category="${escapeAttribute(categoryLabel)}"]`;
  const input = cashflowSettlementCategoryList.querySelector(selector);
  if (input) {
    input.value = breakdown[categoryLabel] ? formatNumber(breakdown[categoryLabel]) : "";
  }
}

function refreshCashflowBusinessBalanceSummary(meta = getMonthMeta(currentMonth)) {
  const settlementPendingTotal = calculateSettlementPendingTotal(currentMonth);
  const businessBalance = (meta.cashflowBusinessAccount || 0) + settlementPendingTotal + (meta.cashflowOtherIncome || 0);
  const totalExpenses = DFY_CASHFLOW_CATEGORY_ORDER.reduce((sum, categoryKey) => sum + getCashflowCategoryTotal(currentMonth, categoryKey), 0);
  const netBalance = businessBalance - totalExpenses;
  const nextMonthTotal = getNextMonthEntriesTotal(currentMonth, "settlementConfirmed") - getNextMonthEntriesTotal(currentMonth, "payables");
  if (document.activeElement !== cashflowBusinessAccountInput) {
    cashflowBusinessAccountInput.value = meta.cashflowBusinessAccount ? formatNumber(meta.cashflowBusinessAccount) : "";
  }
  cashflowSettlementPendingInput.value = settlementPendingTotal ? formatNumber(settlementPendingTotal) : "";
  if (document.activeElement !== cashflowOtherIncomeInput) {
    cashflowOtherIncomeInput.value = meta.cashflowOtherIncome ? formatNumber(meta.cashflowOtherIncome) : "";
  }
  cashflowBusinessBalanceValue.textContent = formatCurrency(businessBalance);
  cashflowNetBalanceValue.textContent = formatSignedCurrency(netBalance);
  cashflowBusinessBalanceFootValue.textContent = formatCurrency(businessBalance);
  if (cashflowNextMonthTotalValue) {
    cashflowNextMonthTotalValue.textContent = formatCurrency(nextMonthTotal);
  }
}

function handleCashflowRecordInput(event) {
  const input = event.target;
  ...
  const record = getCashflowRecordById(currentMonth, currentCashflowCategory, recordId);
  if (!record) {
    return;
  }

  if (field === "amount") {
    record.amount = parseSignedNumericValue(input.value);
  } else if (field === "date") {
    record.date = normalizeInventoryDateKey(input.value) || getDefaultCashflowRecordDate(currentMonth);
  } else if (field === "subcategory") {
    record.subcategory = input.value;
  } else if (field === "note") {
...
function handleCashflowRecordBlur(event) {
  const input = event.target;
  if (!(input instanceof HTMLInputElement) || input.dataset.field !== "amount") {
    return;
  }
  const record = getCashflowRecordById(currentMonth, currentCashflowCategory, input.dataset.recordId);
  if (!record) {
    return;
  }
  input.value = record.amount ? formatNumber(record.amount) : "";
}

function handleCashflowRecordDelete(event) {
  const button = event.target.closest("button[data-record-id]");
  if (!button) {
    return;
  }
  removeCashflowRecord(currentMonth, currentCashflowCategory, button.dataset.recordId);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  renderCashflowPage();
}

function handleNextMonthEntryInput(event) {
  const input = event.target;
  if (!(input instanceof HTMLInputElement)) {
    return;
  }
  const typeKey = normalizeNextMonthTypeKey(input.dataset.nextMonthType);
  const recordId = input.dataset.recordId;
  const field = input.dataset.field;
  if (!typeKey || !recordId || !field) {
    return;
  }
  const record = getNextMonthEntryById(currentMonth, typeKey, recordId);
  if (!record) {
    return;
  }
  ...
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  refreshCashflowBusinessBalanceSummary();
}

function handleNextMonthEntryBlur(event) {
  const input = event.target;
  if (!(input instanceof HTMLInputElement) || input.dataset.field !== "amount") {
    return;
  }
  const typeKey = normalizeNextMonthTypeKey(input.dataset.nextMonthType);
  const record = getNextMonthEntryById(currentMonth, typeKey, input.dataset.recordId);
  if (!record) {
    return;
  }
  input.value = record.amount ? formatNumber(record.amount) : "";
...
function handleNextMonthEntryDelete(event) {
  const button = event.target.closest("button[data-next-month-type][data-record-id]");
  if (!button) {
    return;
  }
  const typeKey = normalizeNextMonthTypeKey(button.dataset.nextMonthType);
  removeNextMonthEntry(currentMonth, typeKey, button.dataset.recordId);
  saveState(`자동 저장됨 · ${formatTime(new Date())}`);
  renderCashflowPage();
}
...
function getMonthMeta(monthKey) {
  ...
  if (!state.meta[monthKey]) {
    state.meta[monthKey] = {
      expectedCost: 0,
      productAverageDivisor: 0,
      cashflowBusinessAccount: 0,
      cashflowSettlementPending: 0,
      cashflowOtherIncome: 0,
      cashflowPayables: 0,
      cashflowSettlementConfirmed: 0,
      cashflowSettlementPendingBreakdown: normalizeSettlementPendingBreakdown(),
      cashflowNextMonthPayablesEntries: [],
      cashflowNextMonthSettlementConfirmedEntries: []
    };
  }
  if (!("productAverageDivisor" in state.meta[monthKey])) {
    state.meta[monthKey].productAverageDivisor = 0;
  }
  if (!("cashflowBusinessAccount" in state.meta[monthKey])) {
    state.meta[monthKey].cashflowBusinessAccount = 0;
  }
  if (!("cashflowSettlementPending" in state.meta[monthKey])) {
    state.meta[monthKey].cashflowSettlementPending = 0;
  }
  if (!("cashflowOtherIncome" in state.meta[monthKey])) {
    state.meta[monthKey].cashflowOtherIncome = 0;
  }
  if (!("cashflowPayables" in state.meta[monthKey])) {
    state.meta[monthKey].cashflowPayables = 0;
  }
  if (!("cashflowSettlementConfirmed" in state.meta[monthKey])) {
    state.meta[monthKey].cashflowSettlementConfirmed = 0;
  }
  state.meta[monthKey].cashflowNextMonthPayablesEntries = normalizeNextMonthEntries(
    state.meta[monthKey].cashflowNextMonthPayablesEntries,
    monthKey,
    "payables",
    state.meta[monthKey].cashflowPayables
  );
  state.meta[monthKey].cashflowNextMonthSettlementConfirmedEntries = normalizeNextMonthEntries(
    state.meta[monthKey].cashflowNextMonthSettlementConfirmedEntries,
    monthKey,
    "settlement-confirmed",
    state.meta[monthKey].cashflowSettlementConfirmed
  );
  state.meta[monthKey].cashflowPayables = 0;
  state.meta[monthKey].cashflowSettlementConfirmed = 0;
  state.meta[monthKey].cashflowSettlementPendingBreakdown = normalizeSettlementPendingBreakdown(
    state.meta[monthKey].cashflowSettlementPendingBreakdown
  );
  return state.meta[monthKey];
}

function normalizeSettlementPendingBreakdown(value) {
  const source = value && typeof value === "object" ? value : {};
  return Object.fromEntries(
    DFY_SETTLEMENT_PENDING_CATEGORIES.map((label) => [label, Math.max(0, Number(source[label]) || 0)])
  );
}

function calculateSettlementPendingTotal(monthKey) {
  const meta = getMonthMeta(monthKey);
  const breakdown = normalizeSettlementPendingBreakdown(meta.cashflowSettlementPendingBreakdown);
  const breakdownTotal = Object.values(breakdown).reduce((sum, amount) => sum + Math.max(0, Number(amount) || 0), 0);
  return breakdownTotal > 0 ? breakdownTotal : Math.max(0, Number(meta.cashflowSettlementPending) || 0);
}

function normalizeNextMonthEntries(value, monthKey, typeKey, legacyAmount = 0) {
  const source = Array.isArray(value) ? value : [];
  const normalized = source
    .map((record, index) => normalizeNextMonthEntry(record, monthKey, `${typeKey}-${index}`))
    .filter(Boolean);
  if (normalized.length) {
    return normalized;
  }
  const numericLegacyAmount = Number(legacyAmount) || 0;
  if (!numericLegacyAmount) {
    return [];
  }
  return [normalizeNextMonthEntry({ amount: numericLegacyAmount, note: "" }, monthKey, `${typeKey}-legacy`)];
}

function normalizeNextMonthEntry(record, monthKey, fallbackId) {
  return {
    id: String(record?.id || createCashflowRecordId(monthKey, fallbackId)),
    amount: Number(record?.amount) || 0,
    note: String(record?.note || "")
  };
}

function normalizeCashflowState(value) {
  const source = value && typeof value === "object" ? value : {};
  return Object.fromEntries(
    Object.entries(source).map(([monthKey, monthValue]) => [monthKey, {
      categories: Object.fromEntries(
        DFY_CASHFLOW_CATEGORY_ORDER.map((categoryKey) => [
          categoryKey,
          Array.isArray(monthValue?.categories?.[categoryKey])
            ? monthValue.categories[categoryKey].map((record, index) => normalizeCashflowRecord(record, monthKey, `${categoryKey}-${index}`))
            : []
        ])
      )
    }])
...
function normalizeCashflowRecord(record, monthKey, fallbackId) {
  const categoryKey = String(fallbackId || "").split("-")[0];
  const subcategories = DFY_CASHFLOW_CATEGORIES[categoryKey]?.subcategories || [];
  return {
    id: String(record?.id || `${monthKey}-${fallbackId}-${Date.now()}`),
    date: normalizeInventoryDateKey(record?.date) || getDefaultCashflowRecordDate(monthKey),
    amount: Number(record?.amount) || 0,
    note: String(record?.note || ""),
    subcategory: String(record?.subcategory || subcategories[0] || "")
  };
}

function ensureCashflowMonthData(monthKey) {
  state.cashflow = state.cashflow || {};
  if (!state.cashflow[monthKey]) {
    state.cashflow[monthKey] = { categories: {} };
  }
  if (!state.cashflow[monthKey].categories) {
    state.cashflow[monthKey].categories = {};
  }
  DFY_CASHFLOW_CATEGORY_ORDER.forEach((categoryKey) => {
    if (!Array.isArray(state.cashflow[monthKey].categories[categoryKey])) {
      state.cashflow[monthKey].categories[categoryKey] = [];
    }
  });
  return state.cashflow[monthKey];
}

function getCashflowRecords(monthKey, categoryKey) {
  return ensureCashflowMonthData(monthKey).categories[categoryKey] || [];
}

function getCashflowCategoryTotal(monthKey, categoryKey) {
  return getCashflowRecords(monthKey, categoryKey).reduce((sum, record) => sum + (Number(record.amount) || 0), 0);
}

function getNextMonthEntries(monthKey, typeKey) {
  const meta = getMonthMeta(monthKey);
  if (typeKey === "payables") {
    return meta.cashflowNextMonthPayablesEntries || [];
  }
  if (typeKey === "settlementConfirmed") {
    return meta.cashflowNextMonthSettlementConfirmedEntries || [];
  }
  return [];
}

function getNextMonthEntriesTotal(monthKey, typeKey) {
  return getNextMonthEntries(monthKey, typeKey).reduce((sum, record) => sum + (Number(record.amount) || 0), 0);
}

function getNextMonthEntryById(monthKey, typeKey, recordId) {
  return getNextMonthEntries(monthKey, typeKey).find((record) => record.id === recordId) || null;
}

function appendNextMonthEntry(monthKey, typeKey) {
  const records = getNextMonthEntries(monthKey, typeKey);
  records.push({
    id: createCashflowRecordId(monthKey, `next-month-${typeKey}`),
    amount: 0,
    note: ""
  });
}

function removeNextMonthEntry(monthKey, typeKey, recordId) {
  const meta = getMonthMeta(monthKey);
  const nextRecords = getNextMonthEntries(monthKey, typeKey).filter((record) => record.id !== recordId);
  if (typeKey === "payables") {
    meta.cashflowNextMonthPayablesEntries = nextRecords;
  } else if (typeKey === "settlementConfirmed") {
    meta.cashflowNextMonthSettlementConfirmedEntries = nextRecords;
  }
}

function getCashflowRecordById(monthKey, categoryKey, recordId) {
  return getCashflowRecords(monthKey, categoryKey).find((record) => record.id === recordId) || null;
}

function appendCashflowRecord(monthKey, categoryKey) {
  const records = getCashflowRecords(monthKey, categoryKey);
  const subcategories = DFY_CASHFLOW_CATEGORIES[categoryKey]?.subcategories || [];
  records.push({
    id: createCashflowRecordId(monthKey, categoryKey),
    date: getDefaultCashflowRecordDate(monthKey),
    amount: 0,
    note: "",
...
function removeCashflowRecord(monthKey, categoryKey, recordId) {
  const records = getCashflowRecords(monthKey, categoryKey);
  const nextRecords = records.filter((record) => record.id !== recordId);
  ensureCashflowMonthData(monthKey).categories[categoryKey] = nextRecords;
}

function createCashflowRecordId(monthKey, categoryKey) {
  return `${monthKey}-${categoryKey}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}

function getDefaultCashflowRecordDate(monthKey) {
...
function getProductRows() { return buildProductRows(getProductCategories()); }
function getProductRowsForCategory(categoryKey) { return getProductRows().filter((row) => row.categoryKey === categoryKey); }
function getProductItemKeyFromRowId(rowId) { return String(rowId || "").split(":").slice(0, 2).join(":"); }
...
function getAverageDivisor(monthKey) {
  const daysInMonth = getDaysInMonth(monthKey);
  const manualDivisor = Math.max(0, Number(getMonthMeta(monthKey).productAverageDivisor) || 0);
  if (manualDivisor > 0) {
    return Math.max(1, Math.min(manualDivisor, daysInMonth));
  }
  return Math.max(1, Math.min(today.getDate(), daysInMonth));
}

function parseCatalogSizePrompt(value, fallbackSizes = ["M", "L"]) {
  const source = String(value || "").trim();
  if (!source) {
    return [...fallbackSizes];
  }
  return normalizeSizeList(source.split(/[,\s/]+/g).filter(Boolean), fallbackSizes);
}

function promptForCatalogItem(options = {}) {
  const name = window.prompt(options.nameMessage || "상품명을 입력해주세요.", options.defaultName || "");
  if (!name || !String(name).trim()) {
    return null;
  }
  const sizes = window.prompt(options.sizeMessage || "사이즈를 입력해주세요. 예: M,L 또는 OS", options.defaultSizes || "M,L");
  if (sizes === null) {
    return null;
  }
  return {
    name: String(name).trim(),
    sizes: parseCatalogSizePrompt(sizes, options.fallbackSizes || ["M", "L"])
  };
}

function persistCustomCatalog(message) {
  state.customCatalog = normalizeCustomCatalog(state.customCatalog);
  saveState(message);
  saveSharedInventoryState(loadSharedInventoryState(), { silentRemote: false });
}

async function handleAddProductItem() {
  const category = getProductCategories()[currentProductCategory];
  const result = promptForCatalogItem({
    nameMessage: `${category.label} 카테고리에 추가할 상품명을 입력해주세요.`,
    defaultSizes: "M,L"
  });
  if (!result) {
    return;
  ...
  const identity = createCatalogIdentity(result.name, result.sizes);
  if (category.items.some((item) => createCatalogIdentity(item.name, item.sizes) === identity)) {
    window.alert("같은 이름/사이즈의 상품이 이미 있습니다.");
    return;
  }

  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  nextCatalog.products[currentProductCategory].push(createProductDefinition(result.name, result.sizes));
  nextCatalog.productOrder[currentProductCategory] = [
    ...getProductCategories()[currentProductCategory].items.map((item) => createCatalogIdentity(item.name, item.sizes)),
    identity
  ].filter((value, index, array) => array.indexOf(value) === index);
  nextCatalog.removedProducts[currentProductCategory] = nextCatalog.removedProducts[currentProductCategory].filter((value) => value !== identity);
  state.customCatalog = nextCatalog;
  selectedProductItemKey = `${currentProductCategory}:${slugify(result.name)}`;
  ensureProductMonthData(currentMonth);
  persistCustomCatalog(`상품 추가 저장됨 · ${formatTime(new Date())}`);
  renderProductPage();
  renderInventoryOverviewPage();
}

async function handleRemoveProductItem() {
  const rows = getProductRowsForCategory(currentProductCategory);
  const selectedRows = rows.filter((row) => row.itemKey === selectedProductItemKey);
  if (!selectedRows.length) {
    window.alert("삭제할 상품 행을 먼저 선택해주세요.");
    return;
  }

  const itemName = selectedRows[0].name;
  const itemSizes = Array.from(new Set(selectedRows.map((row) => row.size)));
  if (!window.confirm(`${itemName} 상품을 삭제하시겠습니까? 기존 판매기록도 함께 정리됩니다.`)) {
    return;
  }

  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  const identity = createCatalogIdentity(itemName, itemSizes);
  nextCatalog.products[currentProductCategory] = nextCatalog.products[currentProductCategory]
    .filter((item) => createCatalogIdentity(item.name, item.sizes) !== identity);
  nextCatalog.productOrder[currentProductCategory] = nextCatalog.productOrder[currentProductCategory]
    .filter((value) => value !== identity);
  if (!nextCatalog.removedProducts[currentProductCategory].includes(identity)) {
    nextCatalog.removedProducts[currentProductCategory].push(identity);
  }
  state.customCatalog = nextCatalog;

  const rowIds = selectedRows.map((row) => row.rowId);
  removeProductLedgerRowsFromState(rowIds);
  selectedProductItemKey = "";
  persistCustomCatalog(`상품 삭제 저장됨 · ${formatTime(new Date())}`);
  renderProductPage();
  renderInventoryOverviewPage();
  if (remoteBootstrapComplete) {
    await deleteProductLedgerRowsFromSupabase(rowIds);
  }
}
...
async function handleAddInventoryItem() {
  const season = getCurrentInventorySeasonConfig();
  const result = promptForCatalogItem({
    nameMessage: `${season.inventoryCategory} · ${season.label} 시즌에 추가할 상품명을 입력해주세요.`,
    defaultSizes: "M,L"
  });
  if (!result) {
    return;
  }
  addInventoryCatalogItem(currentInventoryCategory, currentInventorySeason, result);
}

async function handleAddInventorySeason() {
  const category = getInventoryCategories()[currentInventoryCategory];
  if (!category) {
    return;
  }
  const seasonLabel = window.prompt(`${category.label} 카테고리에 추가할 시즌명을 입력해주세요. 예: 27SS`, "");
  const normalizedLabel = String(seasonLabel || "").trim();
  if (!normalizedLabel) {
    return;
  }
  if (Object.values(category.seasons).some((season) => season.label.toLocaleLowerCase("ko") === normalizedLabel.toLocaleLowerCase("ko"))) {
    window.alert("같은 이름의 시즌이 이미 있습니다.");
    return;
  }

  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  const seasonKey = createUniqueCustomSeasonKey(currentInventoryCategory, normalizedLabel, category.seasons);
  nextCatalog.seasons[currentInventoryCategory][seasonKey] = {
    label: normalizedLabel,
    inventoryCategory: category.label,
    inventorySeason: normalizedLabel
  };
  nextCatalog.inventory[currentInventoryCategory][seasonKey] = [];
  nextCatalog.removedInventory[currentInventoryCategory][seasonKey] = [];
  nextCatalog.inventoryOrder[currentInventoryCategory][seasonKey] = [];
  state.customCatalog = nextCatalog;
  currentInventorySeason = seasonKey;
  selectedInventoryItemId = "";
  selectedInventoryOverviewItemId = "";
  selectedInventoryOverviewSeasonKey = seasonKey;

  const inventoryState = ensureInventoryItemsExist();
  saveState(`시즌 추가 저장됨 · ${formatTime(new Date())}`);
  saveSharedInventoryState(inventoryState, { silentRemote: false });
  renderInventoryPage();
  renderInventoryOverviewPage();
}

async function handleRemoveInventorySeason() {
  const categories = getInventoryCategories();
  const category = categories[currentInventoryCategory];
  const season = category?.seasons?.[currentInventorySeason];
  if (!season) {
    window.alert("삭제할 시즌을 먼저 선택해주세요.");
    return;
  }
  if (Object.keys(category.seasons).length <= 1) {
    window.alert("카테고리에는 최소 한 개의 시즌이 있어야 합니다.");
    return;
  }
  if (!window.confirm(`${category.label} · ${season.label} 시즌을 삭제하시겠습니까? 해당 시즌의 재고와 입출고 기록도 함께 삭제됩니다.`)) {
    return;
  }

  const removedItemIds = new Set(season.items.map((item) => (
    createInventoryItemId(season.inventorySeason, season.inventoryCategory, item.inventoryName)
  )));
  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  if (!nextCatalog.removedSeasons[currentInventoryCategory].includes(currentInventorySeason)) {
    nextCatalog.removedSeasons[currentInventoryCategory].push(currentInventorySeason);
  }
  delete nextCatalog.seasons[currentInventoryCategory][currentInventorySeason];
  delete nextCatalog.inventory[currentInventoryCategory][currentInventorySeason];
  delete nextCatalog.inventoryOrder[currentInventoryCategory][currentInventorySeason];
  delete nextCatalog.removedInventory[currentInventoryCategory][currentInventorySeason];
  state.customCatalog = nextCatalog;

  const inventoryState = loadSharedInventoryState();
  inventoryState.items = inventoryState.items.filter((item) => !removedItemIds.has(item.id));
  inventoryState.transactions = getInventoryTransactions(inventoryState)
    .filter((transaction) => !removedItemIds.has(transaction.itemId));
  selectedInventoryItemId = "";
  selectedInventoryOverviewItemId = "";
  selectedInventoryOverviewSeasonKey = "";
  currentInventorySeason = getDefaultInventorySeasonKey(currentInventoryCategory, nextCatalog);

  saveState(`시즌 삭제 저장됨 · ${formatTime(new Date())}`);
  saveSharedInventoryState(inventoryState, { silentRemote: false });
  renderInventoryPage();
  renderInventoryOverviewPage();
}

function createUniqueCustomSeasonKey(categoryKey, label, existingSeasons = {}) {
  const baseSlug = slugify(label) || "season";
  const prefix = `custom-${baseSlug}`;
  const occupiedKeys = new Set([
    ...Object.keys(existingSeasons || {}),
    ...Object.keys(state.customCatalog?.seasons?.[categoryKey] || {})
  ]);
  let seasonKey = prefix;
  let suffix = 2;
  while (occupiedKeys.has(seasonKey)) {
    seasonKey = `${prefix}-${suffix}`;
    suffix += 1;
  }
  return seasonKey;
}
...
async function handleRemoveInventoryItem() {
  ...
  removeInventoryCatalogItem(currentInventoryCategory, currentInventorySeason, selectedInventoryItemId);
}

async function handleAddInventoryOverviewItem(seasonKey) {
  const category = getInventoryCategories()[currentInventoryOverviewCategory];
  const season = category?.seasons?.[seasonKey];
  if (!season) {
    return;
  }
  const result = promptForCatalogItem({
    nameMessage: `${category.label} · ${season.label} 시즌에 추가할 상품명을 입력해주세요.`,
    defaultSizes: "M,L"
  });
  if (!result) {
    return;
  }
  addInventoryCatalogItem(currentInventoryOverviewCategory, seasonKey, result, { fromOverview: true });
}

async function handleRemoveInventoryOverviewItem(seasonKey) {
  if (!selectedInventoryOverviewItemId || selectedInventoryOverviewSeasonKey !== seasonKey) {
    window.alert("삭제할 상품 행을 먼저 선택해주세요.");
    return;
  }
  removeInventoryCatalogItem(currentInventoryOverviewCategory, seasonKey, selectedInventoryOverviewItemId, { fromOverview: true });
}

function addInventoryCatalogItem(categoryKey, seasonKey, result, options = {}) {
  const categories = getInventoryCategories();
  const season = categories[categoryKey]?.seasons?.[seasonKey];
  if (!season) {
    return;
  }
  const identity = createCatalogIdentity(result.name, result.sizes);
  if (season.items.some((item) => createCatalogIdentity(item.name, item.sizes) === identity)) {
    window.alert("같은 이름/사이즈의 상품이 이미 있습니다.");
    return;
  }

  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  nextCatalog.inventory[categoryKey][seasonKey].push(createInventoryDefinition(result.name, result.sizes, {
    defaultStock: Object.fromEntries(result.sizes.map((size) => [size, 0]))
  }));
  nextCatalog.inventoryOrder[categoryKey][seasonKey] = [
    ...season.items.map((item) => createCatalogIdentity(item.name, item.sizes)),
    identity
  ].filter((value, index, array) => array.indexOf(value) === index);
  nextCatalog.removedInventory[categoryKey][seasonKey] = nextCatalog.removedInventory[categoryKey][seasonKey].filter((value) => value !== identity);
  const productCategory = getProductCategories()[categoryKey] || { items: [] };
  if (!productCategory.items.some((item) => createCatalogIdentity(item.name, item.sizes) === identity)) {
    nextCatalog.products[categoryKey].push(createProductDefinition(result.name, result.sizes));
    nextCatalog.productOrder[categoryKey] = [
      ...productCategory.items.map((item) => createCatalogIdentity(item.name, item.sizes)),
      identity
    ].filter((value, index, array) => array.indexOf(value) === index);
    nextCatalog.removedProducts[categoryKey] = nextCatalog.removedProducts[categoryKey].filter((value) => value !== identity);
  }
  state.customCatalog = nextCatalog;
  selectedInventoryItemId = createInventoryItemId(season.inventorySeason, season.inventoryCategory, result.name);
  selectedInventoryOverviewItemId = selectedInventoryItemId;
  selectedInventoryOverviewSeasonKey = seasonKey;
  const inventoryState = ensureInventoryItemsExist();
  ensureProductMonthData(currentMonth);
  saveState(`재고 상품 추가 저장됨 · ${formatTime(new Date())}`);
  saveSharedInventoryState(inventoryState, { silentRemote: false });
  renderInventoryPage();
  renderInventoryOverviewPage();
  renderProductPage();
...
function removeInventoryCatalogItem(categoryKey, seasonKey, itemId, options = {}) {
  const categories = getInventoryCategories();
  const season = categories[categoryKey]?.seasons?.[seasonKey];
  if (!season) {
    return;
  ...
  const inventoryState = ensureInventoryItemsExist();
  const record = inventoryState.items.find((item) => item.id === itemId);
  if (!record) {
    window.alert("삭제할 상품 정보를 찾지 못했습니다.");
    return;
  }
  if (!window.confirm(`${record.name} 상품을 삭제하시겠습니까? 관련 재고기록도 함께 정리됩니다.`)) {
    return;
  }

  const identity = createCatalogIdentity(record.name, record.sizes);
  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  nextCatalog.inventory[categoryKey][seasonKey] = nextCatalog.inventory[categoryKey][seasonKey]
    .filter((item) => createCatalogIdentity(item.name, item.sizes) !== identity);
  nextCatalog.inventoryOrder[categoryKey][seasonKey] = nextCatalog.inventoryOrder[categoryKey][seasonKey]
    .filter((value) => value !== identity);
  if (!nextCatalog.removedInventory[categoryKey][seasonKey].includes(identity)) {
    nextCatalog.removedInventory[categoryKey][seasonKey].push(identity);
  }
  state.customCatalog = nextCatalog;

  inventoryState.items = inventoryState.items.filter((item) => item.id !== itemId);
  inventoryState.transactions = getInventoryTransactions(inventoryState).filter((transaction) => transaction.itemId !== itemId);
  selectedInventoryItemId = "";
  if (options.fromOverview) {
    selectedInventoryOverviewItemId = "";
    selectedInventoryOverviewSeasonKey = "";
  }
  saveState(`재고 상품 삭제 저장됨 · ${formatTime(new Date())}`);
  saveSharedInventoryState(inventoryState, { silentRemote: false });
  renderInventoryPage();
  renderInventoryOverviewPage();
}

function removeProductLedgerRowsFromState(rowIds) {
  const rowIdSet = new Set(rowIds);
  const pendingMap = loadPendingProductSyncMap();

  Object.keys(state.productLedger || {}).forEach((monthKey) => {
    const monthLedger = state.productLedger[monthKey];
    if (!monthLedger || typeof monthLedger !== "object") {
      return;
    }
    rowIds.forEach((rowId) => {
      if (rowId in monthLedger) {
        delete monthLedger[rowId];
      }
    });
    if (!Object.keys(monthLedger).length) {
      delete state.productLedger[monthKey];
    }
  });

  Object.keys(pendingMap).forEach((entryKey) => {
    const { rowId } = parseRemoteProductEntryKey(entryKey);
...
function reorderProductItemsByDrag(sourceItemKey, targetItemKey, clientY, rect) {
  if (!sourceItemKey || !targetItemKey || sourceItemKey === targetItemKey) {
    return;
  }
  const category = getProductCategories()[currentProductCategory];
  const orderedKeys = category.items.map((item) => `${currentProductCategory}:${slugify(item.name)}`);
  const sourceIndex = orderedKeys.indexOf(sourceItemKey);
  const targetIndex = orderedKeys.indexOf(targetItemKey);
  if (sourceIndex === -1 || targetIndex === -1) {
    return;
  }
  const insertAfter = clientY > (rect.top + rect.height / 2);
  const [moved] = orderedKeys.splice(sourceIndex, 1);
  const baseIndex = orderedKeys.indexOf(targetItemKey);
  const nextIndex = insertAfter ? baseIndex + 1 : baseIndex;
  orderedKeys.splice(nextIndex, 0, moved);

  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  nextCatalog.productOrder[currentProductCategory] = orderedKeys.map((itemKey) => {
    const item = category.items.find((entry) => `${currentProductCategory}:${slugify(entry.name)}` === itemKey);
    return item ? createCatalogIdentity(item.name, item.sizes) : "";
  }).filter(Boolean);
  state.customCatalog = nextCatalog;
  selectedProductItemKey = moved;
  persistCustomCatalog(`상품 순서 저장됨 · ${formatTime(new Date())}`);
  renderProductPage();
}

function reorderInventoryItemsByDrag(categoryKey, seasonKey, sourceItemId, targetItemId, clientY, rect, options = {}) {
  ...
  const inventoryState = ensureInventoryItemsExist();
  const seasonRecords = season.items.map((item, itemIndex) => (
    inventoryState.items.find((entry) => entry.id === createInventoryItemId(season.inventorySeason, season.inventoryCategory, item.inventoryName))
      || createInventoryItemRecord(season, item, itemIndex)
  ));
  const orderedIds = seasonRecords.map((item) => item.id);
  ...
  const baseIndex = orderedIds.indexOf(targetItemId);
  const nextIndex = insertAfter ? baseIndex + 1 : baseIndex;
  orderedIds.splice(nextIndex, 0, moved);

  const nextCatalog = normalizeCustomCatalog(state.customCatalog);
  nextCatalog.inventoryOrder[categoryKey][seasonKey] = orderedIds.map((itemId) => {
    const item = seasonRecords.find((entry) => entry.id === itemId);
    return item ? createCatalogIdentity(item.name, item.sizes) : "";
  }).filter(Boolean);
  state.customCatalog = nextCatalog;
  selectedInventoryItemId = moved;
  selectedInventoryOverviewItemId = moved;
  selectedInventoryOverviewSeasonKey = seasonKey;
  const inventorySnapshot = ensureInventoryItemsExist();
  saveState(`재고 순서 저장됨 · ${formatTime(new Date())}`);
  saveSharedInventoryState(inventorySnapshot, { silentRemote: false });
  renderInventoryPage();
  renderInventoryOverviewPage();
}
...
function normalizeCashflowCategoryKey(value) { return DFY_CASHFLOW_CATEGORY_ORDER.includes(value) ? value : getDefaultCashflowCategoryKey(); }
function getDefaultCashflowCategoryKey() { return DFY_CASHFLOW_CATEGORY_ORDER[0]; }
function normalizeCategoryKey(value, customCatalog = null) {
  const hasBaseCategory = Object.prototype.hasOwnProperty.call(DFY_PRODUCT_CATEGORIES, value);
  const hasCustomCategory = Boolean(customCatalog?.products?.[value]?.length && DFY_INVENTORY_CATEGORIES[value]);
  return hasBaseCategory || hasCustomCategory ? value : getDefaultCategoryKey();
}
...
function normalizeInventorySeasonKey(categoryKey, seasonKey, customCatalog = null) {
  const baseSeasons = DFY_INVENTORY_CATEGORIES[categoryKey]?.seasons || {};
  const customSeasons = customCatalog?.seasons?.[categoryKey] || {};
  const removedSeasons = new Set(customCatalog?.removedSeasons?.[categoryKey] || []);
  return !removedSeasons.has(seasonKey)
    && (Object.prototype.hasOwnProperty.call(baseSeasons, seasonKey) || Object.prototype.hasOwnProperty.call(customSeasons, seasonKey))
    ? seasonKey
    : getDefaultInventorySeasonKey(categoryKey, customCatalog);
}
function getDefaultInventorySeasonKey(categoryKey, customCatalog = null) {
  const removedSeasons = new Set(customCatalog?.removedSeasons?.[categoryKey] || []);
  return Object.keys(DFY_INVENTORY_CATEGORIES[categoryKey]?.seasons || {}).find((seasonKey) => !removedSeasons.has(seasonKey))
    || Object.keys(customCatalog?.seasons?.[categoryKey] || {}).find((seasonKey) => !removedSeasons.has(seasonKey))
    || "";
}
function normalizeInventoryDateKey(value) { return /^\d{4}-\d{2}-\d{2}$/.test(String(value || "")) ? value : ""; }
function normalizeNextMonthTypeKey(value) { return value === "payables" || value === "settlementConfirmed" ? value : ""; }
...
function parseSignedNumericValue(value) {
  ...
  return numeric ? (negative ? -1 : 1) * Number(numeric) : 0;
}
...
async function hydrateFromSupabase() {
  ...
  try {
    ...
    suppressProductBlurCommit = true;
    render();
    window.requestAnimationFrame(() => {
      suppressProductBlurCommit = false;
    });
  } finally {
...
async function fetchProductLedgerRowsFromSupabase(options = {}) {
  if (!supabaseClient) {
  ...
  const monthKeys = Array.isArray(options.monthKeys) ? options.monthKeys.filter(Boolean) : [];
  let from = 0;
  ...
  while (true) {
    let query = supabaseClient
      ...
      .range(from, from + DFY_SUPABASE_PAGE_SIZE - 1);
    ...
    rows.push(...(data || []));
    if (!data || data.length < DFY_SUPABASE_PAGE_SIZE) {
      break;
    }
    from += DFY_SUPABASE_PAGE_SIZE;
  }

  return { data: rows, error: null };
}

async function savePendingProductEntriesToSupabase(entries) {
  if (!supabaseClient) {
    return false;
  }
  ...
  entries.forEach((entry) => {
    if (!entry?.dateKey || !entry?.rowId) {
      return;
    }
    entryMap.set(createRemoteProductEntryKey(entry.dateKey, entry.rowId), {
      monthKey: entry.monthKey || entry.dateKey.slice(0, 7),
  ...
  const deleteEntries = normalizedEntries.filter((entry) => entry.quantity <= 0);

  if (upsertPayload.length) {
    for (const payloadChunk of chunkArray(upsertPayload, DFY_SUPABASE_WRITE_CHUNK_SIZE)) {
      const { error } = await supabaseClient
        .from(DFY_REMOTE_PRODUCT_TABLE)
        .upsert(payloadChunk, { onConflict: "date_key,row_id" });

      if (error) {
        console.error("Supabase product upsert failed", error, payloadChunk);
  ...
  if (upsertPayload.length) {
    const monthKeys = [...new Set(upsertPayload.map((entry) => entry.month_key))];
    const { data: savedRows, error: verifyError } = await fetchProductLedgerRowsFromSupabase({
      monthKeys,
      columns: "date_key,row_id,quantity"
    });

    if (verifyError) {
      console.error("Supabase product verify failed", verifyError, upsertPayload);
      saveIndicator.textContent = `Supabase 상품확인 실패 · ${verifyError.message || "오류"}`;
      return false;
    }

    const savedMap = new Map((savedRows || []).map((record) => [
      createRemoteProductEntryKey(record.date_key, record.row_id),
      Math.max(0, Number(record.quantity) || 0)
    ]));
    const missingEntry = upsertPayload.find((entry) => {
      const key = createRemoteProductEntryKey(entry.date_key, entry.row_id);
      return savedMap.get(key) !== entry.quantity;
    });

    if (missingEntry) {
      console.error("Supabase product verify mismatch", missingEntry, savedRows);
      saveIndicator.textContent = "Supabase 상품확인 실패 · 저장값 불일치";
      return false;
    }
  }
  ...
  for (const entry of deleteEntries) {
    const { error: deleteError } = await supabaseClient
      ...
      .eq("row_id", entry.rowId);

    if (deleteError) {
      console.error("Supabase product delete failed", deleteError, entry);
      saveIndicator.textContent = `Supabase 상품저장 실패 · ${deleteError.message || "오류"}`;
      return false;
    }
  }
...
async function handleProductSaveButtonClick() {
  const activeElement = document.activeElement;
  if (
    activeElement instanceof HTMLInputElement
    && activeElement.classList.contains("product-input")
    && activeElement.dataset.rowId
    && activeElement.dataset.date
  ) {
    updateProductCellFromInput(activeElement, false);
  }

  const productEntriesToSave = collectProductEntriesForSave(currentMonth);

  if (!supabaseClient) {
    saveIndicator.textContent = `Supabase 연결 없음 · ${formatTime(new Date())}`;
    return;
  }

  if (!productEntriesToSave.length) {
    saveIndicator.textContent = `저장할 변경사항 없음 · ${formatTime(new Date())}`;
    updateProductSaveButton();
    return;
  }

  isProductSaveInFlight = true;
  updateProductSaveButton();
  saveIndicator.textContent = `Supabase 상품저장 중 · ${formatTime(new Date())}`;

  const isSaved = await savePendingProductEntriesToSupabase(productEntriesToSave);
  isProductSaveInFlight = false;
  ...
  clearPendingProductSyncEntries(productEntriesToSave.map((entry) => createRemoteProductEntryKey(entry.dateKey, entry.rowId)));
  clearDirtyProductMonth(currentMonth);
  updateProductSaveButton();
  saveIndicator.textContent = `Supabase 상품저장 완료 · ${formatTime(new Date())}`;
  saveState(saveIndicator.textContent);
  refreshProductFooterTotals();
  renderProductCategoryChart();
  window.setTimeout(() => {
    if (document.hidden || hasActiveRemoteEditFocus() || hasPendingRemoteWrites()) {
      return;
    }
    void hydrateFromSupabase();
  }, 1000);
...
async function hydrateProductLedgerFromSupabase() {
  const { data, error } = await fetchProductLedgerRowsFromSupabase();
  ...
  getPendingProductSyncEntries().forEach((entry) => {
    if (!nextLedger[entry.monthKey]) {
      nextLedger[entry.monthKey] = {};
    }
    if (!nextLedger[entry.monthKey][entry.rowId]) {
      nextLedger[entry.monthKey][entry.rowId] = {};
    }
    if (entry.quantity > 0) {
      nextLedger[entry.monthKey][entry.rowId][entry.dateKey] = entry.quantity;
    } else {
      delete nextLedger[entry.monthKey][entry.rowId][entry.dateKey];
    }
  });
...
async function upsertProductLedgerToSupabase(entryKeys) {
  if (!supabaseClient || !entryKeys.length) return false;
...
async function deleteProductLedgerRowsFromSupabase(rowIds) {
  if (!supabaseClient || !rowIds.length) {
    return false;
  }

  const uniqueRowIds = Array.from(new Set(rowIds.map((rowId) => String(rowId || "")).filter(Boolean)));
  ...
  if (error) {
    console.error("Supabase product delete failed", error, uniqueRowIds);
    saveIndicator.textContent = `Supabase 상품삭제 실패 · ${error.message || "오류"}`;
    return false;
  }
...
async function hydrateInventoryStateFromSupabase() {
  const pendingInventoryEntry = loadPendingInventorySyncEntry();
  const pendingInventoryState = pendingInventoryEntry?.state || null;
  ...
  if (!data) {
    const stateToUpload = pendingInventoryState || localInventoryState;
    state.customCatalog = normalizeCustomCatalog(stateToUpload.customCatalog || state.customCatalog);
    if (stateToUpload.items.length || stateToUpload.transactions.length || hasCustomCatalogData(stateToUpload.customCatalog)) {
      localStorage.setItem(DFY_INVENTORY_STORAGE_KEY, JSON.stringify(stateToUpload));
      queueRemoteInventorySave(stateToUpload, { silentRemote: true });
    }
    return;
  }
  ...
  const remoteInventoryState = normalizeSharedInventoryState({
    ...
    customCatalog: data.meta?.customCatalog || null
  });
  const remoteUpdatedAt = Number(new Date(data.updated_at || 0)) || 0;
  const pendingChangedAt = Number(new Date(pendingInventoryEntry?.changedAt || 0)) || 0;

  if (pendingInventoryState) {
    const mergedCatalog = mergeCustomCatalog(remoteInventoryState.customCatalog, pendingInventoryState.customCatalog);
    if (areInventoryStatesEqual(remoteInventoryState, pendingInventoryState)
      && areInventoryStatesEqual(
        { ...remoteInventoryState, customCatalog: mergedCatalog },
      ...
      state.customCatalog = normalizeCustomCatalog(remoteInventoryState.customCatalog);
      return;
    }
    if (remoteUpdatedAt && pendingChangedAt && remoteUpdatedAt >= pendingChangedAt) {
      clearPendingInventorySyncState();
      const mergedRemoteState = normalizeSharedInventoryState({
        ...remoteInventoryState,
        customCatalog: mergedCatalog
      });
      localStorage.setItem(DFY_INVENTORY_STORAGE_KEY, JSON.stringify(mergedRemoteState));
      state.customCatalog = mergedCatalog;
      if (!areInventoryStatesEqual(mergedRemoteState, remoteInventoryState)) {
        queueRemoteInventorySave(mergedRemoteState, { silentRemote: true });
      }
      return;
    }
    const mergedPendingState = normalizeSharedInventoryState({
      ...pendingInventoryState,
      customCatalog: mergedCatalog
    });
    localStorage.setItem(DFY_INVENTORY_STORAGE_KEY, JSON.stringify(mergedPendingState));
    state.customCatalog = mergedCatalog;
    queueRemoteInventorySave(mergedPendingState, { silentRemote: true });
    return;
  }

  const mergedCatalog = mergeCustomCatalog(remoteInventoryState.customCatalog, localInventoryState.customCatalog);
  const mergedRemoteState = normalizeSharedInventoryState({
    ...remoteInventoryState,
    customCatalog: mergedCatalog
  });
...
async function upsertInventoryStateToSupabase(inventoryState, options = {}) {
  if (!supabaseClient) return;
  const { data: remoteData, error: remoteReadError } = await supabaseClient
    .from(DFY_REMOTE_INVENTORY_TABLE)
    .select("meta")
    .eq("state_key", DFY_REMOTE_INVENTORY_KEY)
    .maybeSingle();
  if (remoteReadError) {
    console.error("Supabase inventory catalog merge failed", remoteReadError);
  }
  const mergedCatalog = mergeCustomCatalog(
    remoteData?.meta?.customCatalog || null,
    inventoryState.customCatalog || state.customCatalog
  );
  const mergedInventoryState = normalizeSharedInventoryState({
    ...inventoryState,
    customCatalog: mergedCatalog
  });
  inventoryState.customCatalog = mergedCatalog;
  state.customCatalog = mergedCatalog;
  const payload = {
    state_key: DFY_REMOTE_INVENTORY_KEY,
    items: mergedInventoryState.items,
    transactions: mergedInventoryState.transactions,
    meta: {
  ...
  localStorage.setItem(DFY_INVENTORY_STORAGE_KEY, JSON.stringify(mergedInventoryState));
  clearPendingInventorySyncState();
  if (!options.silentRemote) {
...
function hasActiveRemoteEditFocus() {
  ...
  if (
    ...
    return activeElement.classList.contains("table-input")
      || activeElement.classList.contains("product-input")
      || activeElement.classList.contains("inventory-input")
      || activeElement.classList.contains("inventory-edit-input")
      || activeElement.classList.contains("cashflow-record-input")
      || activeElement.classList.contains("summary-inline-input")
      || activeElement.classList.contains("product-divisor-input")
      || activeElement === expectedCostInput
      || activeElement === productAverageDivisor
      || activeElement === cashflowBusinessAccountInput
      || activeElement === cashflowOtherIncomeInput;
  }
...
function hasPendingRemoteWrites() {
  return Boolean(
    remoteSaveTimer
    || remoteProductSaveTimer
    || remoteInventorySaveTimer
    || isProductSaveInFlight
  );
}

function startProductRealtimeSync() {
  if (!supabaseClient || typeof supabaseClient.channel !== "function" || productRealtimeChannel) {
    return;
  }

  productRealtimeChannel = supabaseClient
    .channel("dfy-product-ledger-sync")
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: DFY_REMOTE_PRODUCT_TABLE },
      () => {
        if (document.hidden) {
          return;
        }
        if (hasActiveRemoteEditFocus()) {
          return;
        }
