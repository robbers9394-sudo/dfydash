...
async function hydrateProductLedgerFromSupabase() {
  ...
  data.forEach((record) => {
    ...
    const entryKey = createRemoteProductEntryKey(dateKey, rowId);
    remoteEntryMap.set(entryKey, {
      monthKey,
      dateKey,
      rowId,
      quantity,
      updatedAt: Number(new Date(record.updated_at || 0)) || 0
    });
    ...
    if (pendingEntry && pendingEntry.quantity !== quantity) {
      const isWithinPendingGrace = pendingChangedAt && (Date.now() - pendingChangedAt) < DFY_PRODUCT_PENDING_GRACE_MS;
      if (isWithinPendingGrace && (!remoteUpdatedAt || pendingChangedAt > remoteUpdatedAt)) {
        return;
      }
  ...
  if (pendingLocalEntries.length) {
    ...
    pendingLocalEntries.forEach((entry) => {
      ...
      const pendingChangedAt = Number(new Date(entry.changedAt || 0)) || 0;
      const isFreshPending = pendingChangedAt && (Date.now() - pendingChangedAt) < DFY_PRODUCT_PENDING_GRACE_MS;
      const remoteEntry = remoteEntryMap.get(entryKey);
      ...
      if (remoteEntry && remoteEntry.quantity === entry.quantity) {
        staleEntryKeys.push(entryKey);
        return;
      }

      staleEntryKeys.push(entryKey);
    });
    clearPendingProductSyncEntries([...confirmedEntryKeys, ...staleEntryKeys]);
