...
async function hydrateProductLedgerFromSupabase() {
  ...
  data.forEach((record) => {
    ...
    remoteEntryMap.set(createRemoteProductEntryKey(dateKey, rowId), quantity);
  });
  ...
  if (pendingLocalEntries.length) {
    ...
    pendingLocalEntries.forEach((entry) => {
      ...
      const remoteQuantity = Number(remoteEntryMap.has(entryKey) ? remoteEntryMap.get(entryKey) : 0);
