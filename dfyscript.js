...
function loadDirtyProductMonths() {
  try {
    const raw = localStorage.getItem(DFY_DIRTY_PRODUCT_MONTHS_KEY);
    if (!raw) {
      return [];
    }
    const parsed = JSON.parse(raw);
...
function saveDirtyProductMonths(months) {
  localStorage.setItem(DFY_DIRTY_PRODUCT_MONTHS_KEY, JSON.stringify(Array.from(new Set(months.map((value) => String(value))))));
}
