...
function loadState() {
  ...
  try {
    ...
    return {
      ...
      meta: parsedMeta,
      productLedger: normalizeProductLedger(parsed.productLedger),
      ui: {
...
function getInventoryTransactions(inventoryState) {
  return Array.isArray(inventoryState.transactions) ? inventoryState.transactions : [];
}
...
function syncInventoryOutflows() {
  try {
    ...
    inventoryState.items.forEach((item) => {
      const key = createInventoryLookupKey(item.category, item.name);
      if (!lookup.has(key)) lookup.set(key, item);
    });
    const retainedTransactions = Array.isArray(inventoryState.transactions)
      ? inventoryState.transactions.filter((transaction) => transaction?.source !== DFY_INVENTORY_SYNC_SOURCE)
      : [];
    const generatedTransactions = [];
    const unmapped = [];
    Object.entries(state.productLedger || {}).forEach(([monthKey, rowMap]) => {
      Object.entries(rowMap || {}).forEach(([rowId, dateMap]) => {
        const row = DFY_PRODUCT_ROW_MAP.get(rowId);
        if (!row) return;
        const inventoryItem = lookup.get(createInventoryLookupKey(row.inventoryCategory, row.inventoryName));
        Object.entries(dateMap || {}).forEach(([dateKey, quantity]) => {
          ...
          if (!inventoryItem) {
            unmapped.push({ monthKey, dateKey, name: row.inventoryName, size: row.size });
            return;
          }
          generatedTransactions.push({
            id: `${DFY_INVENTORY_SYNC_SOURCE}-${dateKey}-${inventoryItem.id}-${row.inventorySize}`,
            itemId: inventoryItem.id,
            date: dateKey,
            type: "out",
            quantity: numericQuantity,
            size: row.inventorySize,
            source: DFY_INVENTORY_SYNC_SOURCE,
...
function hasActiveRemoteEditFocus() {
  ...
  return activeElement instanceof HTMLInputElement
    && (
      activeElement.classList.contains("table-input")
