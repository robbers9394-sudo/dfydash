...
function renderProductLedgerBody() {
  ...
  productLedgerBody.innerHTML = rows.map((row, rowIndex) => {
    ...
    return `<tr class="${rowClass}" data-row-id="${row.rowId}">${row.isFirstSize ? `<th class="product-sticky-col product-name-cell" rowspan="${row.rowSpan}">${escapeHtml(row.name)}</th>` : ""}<th class="product-sticky-col product-size-cell">${escapeHtml(row.size)}</th><td class="product-sticky-col product-total-cell" data-role="row-total">${formatCellValue(rowTotal)}</td><td class="product-sticky-col product-average-cell" data-role="row-average">${formatAverageCell(rowAverage, rowTotal)}</td>${dateKeys.map((dateKey, colIndex) => renderProductInputCell(row.rowId, row.name, row.size, dateKey, rowIndex, colIndex)).join("")}</tr>`;
  }).join("");
}
