...
function getMonthMeta(monthKey) { state.meta = state.meta || {}; if (!state.meta[monthKey]) { state.meta[monthKey] = { expectedCost: 0, productAverageDivisor: 0 }; } if (!("productAverageDivisor" in state.meta[monthKey])) { state.meta[monthKey].productAverageDivisor = 0; } return state.meta[monthKey]; }
...
async function saveProductMonthToSupabase(monthKey) {
  if (!supabaseClient) {
    return false;
  }

  const monthLedger = state.productLedger?.[monthKey] || {};
  const payload = Object.entries(monthLedger).flatMap(([rowId, dateMap]) => (
  ...
  if (deleteError) {
    console.error("Supabase product month delete failed", deleteError, monthKey);
  ...
  if (error) {
    console.error("Supabase product month upsert failed", error, payload);
...
async function handleProductSaveButtonClick() {
  ...
  if (!isDirtyProductMonth(currentMonth)) {
  ...
  const isSaved = await saveProductMonthToSupabase(currentMonth);
...
async function hydrateProductLedgerFromSupabase() {
  ...
  loadDirtyProductMonths().forEach((monthKey) => {
    const localMonthLedger = normalizeProductLedger({ [monthKey]: state.productLedger?.[monthKey] || {} })[monthKey] || {};
    nextLedger[monthKey] = localMonthLedger;
  });
...
function hasPendingRemoteWrites() {
  return Boolean(remoteSaveTimer || remoteProductSaveTimer || remoteInventorySaveTimer || isProductSaveInFlight);
}
