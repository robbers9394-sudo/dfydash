...
function updateProductSaveButton() {
  const pendingCount = getPendingProductSyncEntries().filter((entry) => entry.monthKey === currentMonth).length;
  productSaveButton.disabled = pendingCount === 0 || isProductSaveInFlight;
  if (isProductSaveInFlight) {
    productSaveButton.textContent = "저장 중...";
    return;
  }
  productSaveButton.textContent = pendingCount > 0 ? `저장 ${pendingCount}` : "저장";
}
...
async function handleProductSaveButtonClick() {
  ...
  const pendingEntries = getPendingProductSyncEntries().filter((entry) => entry.monthKey === currentMonth);
  ...
  const entryKeys = pendingEntries.map((entry) => createRemoteProductEntryKey(entry.dateKey, entry.rowId));
  isProductSaveInFlight = true;
  updateProductSaveButton();
  saveIndicator.textContent = `Supabase 상품저장 중 · ${formatTime(new Date())}`;

  const isSaved = await upsertProductLedgerToSupabase(entryKeys);
...
async function hydrateProductLedgerFromSupabase() {
  ...
  const syncedEntryKeys = [];

  if (pendingLocalEntries.length) {
    pendingLocalEntries.forEach((entry) => {
      if (!entry.monthKey || !entry.rowId || !entry.dateKey) {
        return;
      }
      const remoteQuantity = Number(nextLedger?.[entry.monthKey]?.[entry.rowId]?.[entry.dateKey] || 0);
      if (remoteQuantity === entry.quantity) {
        syncedEntryKeys.push(createRemoteProductEntryKey(entry.dateKey, entry.rowId));
        return;
      }
  ...
  clearPendingProductSyncEntries(syncedEntryKeys);
...
async function upsertProductLedgerToSupabase(entryKeys) {
  ...
  clearPendingProductSyncEntries(entryKeys);
  updateProductSaveButton();
  return true;
}
